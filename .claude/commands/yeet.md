...
5. **Run the code reviews.** After lint and type checks pass, run these commands in order:
   - `/code-review xhigh --fix`
   - `/simplify`
   - `/security-review`
   - `/polar-code-review`

   Fix any real issues they surface, then re-run the affected check. Commit the fixes before moving on.

6. **Create the PR as a draft** using the standard PR creation instructions from the system prompt. Use the full commit history from `main` to draft the PR title and description. Pass `--draft` to `gh pr create`.
   ...
   **Write the description short and simple.** Many readers are not native English speakers. Follow these rules:
   - Use short sentences. One idea per sentence.
   - Use simple, common words. Avoid jargon and long phrases.
   - Say what the change does and why. Do not repeat the diff line by line.
   - Prefer short bullets over long paragraphs.
   - A reviewer should read the whole PR body in under a minute.
