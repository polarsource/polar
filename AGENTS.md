...
- `/polar-code-review` — checks the diff against Polar-specific rules with parallel
  lenses (`conventions-check`, `adr-check`, `reuse-check`, `slop-check`, plus
  `api-surface-review` / `ship-safety` / `billing-review` when the paths match). Bugs,
  security, and simplification are covered by the built-in `/code-review`,
