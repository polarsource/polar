...
Files are limited to 250 lines of code (excluding blanks and comments) via the `max-lines` Oxlint rule. If you hit this limit, refactor the file into smaller pieces — extract sub-components, split hooks into separate files, or move helpers into their own modules. Do not add `oxlint-disable max-lines` to new files.
