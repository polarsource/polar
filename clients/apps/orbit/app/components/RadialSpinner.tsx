...
export const RadialSpinner = () => {
  ...
  useEffect(() => {
    ...
    const TAIL_DELAY = 0.16;     // how long after the tip the start begins following
    // Cycle long enough for the last spoke's tail to fully complete
    const FULL_CYCLE = (SPOKE_COUNT - 1) * SPOKE_INTERVAL + PATH_DURATION + TAIL_DELAY;
    ...
    const draw = (now: number) => {
      ...
      for (let i = 0; i < SPOKE_COUNT; i++) {
        ...
        // Trailing start: same bezier, but delayed by TAIL_DELAY
        ...
        if (tailProgress >= 1) continue; // segment has fully passed
