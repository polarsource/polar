...
const CONTEXT = `Polar (polar.sh) is a merchant-of-record payments platform. You are reviewing the hourly production check of its checkout.
The test drives a real Chrome with Stagehand: it opens a checkout link on production for a subscription with a free trial, fills email and cardholder name, sets a US billing address (country, street, postal code, city, state), types a real card into Stripe's Payment Element iframe, clicks "Start trial", expects a redirect to /confirmation, polls the Polar API until the checkout status is "succeeded" (which depends on Stripe's setup_intent.succeeded webhook reaching Polar) and a "trialing" subscription exists, then cancels it through the customer portal API.
