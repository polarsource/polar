...
This data is also available from the [Orders](/docs/api#tag/orders/GET/v1/orders/{id}) and [Subscriptions](/docs/api#tag/subscriptions/GET/v1/customer-portal/subscriptions/{id}) API, under the `custom_field_data` property. Each value is referenced by the **slug** of the field.
