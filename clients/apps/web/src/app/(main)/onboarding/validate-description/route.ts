...
// Loaded lazily from the local copy of the canonical MDX (generated by the
// `copy-aup` prebuild step and traced into the bundle via next.config.mjs). Read
// on first use, and degrade to empty on a missing file rather than throwing at
// module load — a missing copy must never 500 the whole route.
let cachedAupContent: string | null = null
function getAupContent(): string {
  if (cachedAupContent === null) {
    try {
      cachedAupContent = readFileSync(
        join(
          process.cwd(),
