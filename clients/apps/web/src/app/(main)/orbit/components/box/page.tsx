...
const guidelines = [
  {
    rule: 'Pass CSS variable tokens as prop values',
    desc: 'backgroundColor, padding, gap, borderRadius accept CSS variable strings from useOrbit() or orbitTokens. Reach for className only for things tokens cannot express.',
    do: `const { spacing, card } = useOrbit()
...
const props = [
  ...
  },
  {
    ...
    default: '—',
    desc: 'Color tokens from useOrbit(). Values are CSS custom properties — dark mode is automatic.',
  },
  {
  ...
  return (
    <Stack vertical gap={sp['SPACING_14']}>
      <OrbitPageHeader
        label="Component"
        title="Box"
        description="The atomic styling primitive in Orbit. Box is not a flex container — use Stack for flex layouts. Box handles spacing, color, radius, and flex child props (flex, alignSelf) through CSS variable token props."
      />
