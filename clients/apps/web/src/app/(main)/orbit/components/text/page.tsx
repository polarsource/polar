  ...
  return (
    <Stack vertical gap={10}>
          ...
          {variants.map(({ variant, label, desc }) => (
            <div key={variant} className="grid grid-cols-5 items-baseline gap-8 py-5">
              <div className="col-span-1">
                <Text as="code" variant="mono">{label}</Text>
              </div>
      ...
      <Stack vertical gap={4}>
        ...
        <Stack vertical className="dark:divide-polar-800 divide-y divide-neutral-200">
          {colors.map(({ color, label, desc }) => (
            <div key={color} className="grid grid-cols-5 items-baseline gap-8 py-5">
              <div className="col-span-1">
                <Text as="code" variant="mono">{label}</Text>
              </div>
              <div className="col-span-2">
        ...
        <div className="dark:divide-polar-800 divide-y divide-neutral-200 rounded-xl border border-neutral-200 dark:border-polar-700">
          <div className="grid grid-cols-5 gap-4 px-4 py-2">
            <Text variant="caption" className="col-span-1">variant</Text>
            <Text variant="caption">default</Text>
            <Text variant="caption">error</Text>
            <Text variant="caption">warning</Text>
            <Text variant="caption">success</Text>
          </div>
          {(['body', 'label', 'caption', 'subtle', 'mono'] as const).map((v) => (
            <div key={v} className="grid grid-cols-5 gap-4 px-4 py-3">
              <Text as="code" variant="mono" className="col-span-1">{v}</Text>
              <Text variant={v} color="default">Sample</Text>
              <Text variant={v} color="error">Sample</Text>
              <Text variant={v} color="warning">Sample</Text>
              <Text variant={v} color="success">Sample</Text>
            </div>
          ))}
        </div>
      </Stack>
      ...
      <Stack vertical gap={4}>
        ...
        <Stack vertical className="dark:divide-polar-800 divide-y divide-neutral-200">
          {aligns.map(({ align, label }) => (
            <div key={align} className="grid grid-cols-5 items-baseline gap-8 py-5">
              <div className="col-span-1">
                <Text as="code" variant="mono">{label}</Text>
              </div>
      ...
      <Stack vertical gap={4}>
        ...
        <Stack vertical className="dark:divide-polar-800 divide-y divide-neutral-200">
          {wraps.map(({ wrap, label, desc }) => (
            <div key={wrap} className="grid grid-cols-5 items-baseline gap-8 py-5">
              <div className="col-span-1">
                <Text as="code" variant="mono">{label}</Text>
              </div>
      ...
      <Stack vertical gap={4}>
        ...
        />
        <Stack vertical className="dark:divide-polar-800 divide-y divide-neutral-200">
          {(['p', 'span', 'label', 'strong', 'em', 'small', 'code'] as const).map((tag) => (
            <div key={tag} className="grid grid-cols-5 items-baseline gap-8 py-4">
              <div className="col-span-1">
                <Text as="code" variant="mono">{tag}</Text>
              </div>
              <div className="col-span-4">
        ...
        <Stack vertical className="dark:divide-polar-800 divide-y divide-neutral-200">
          {props.map(({ name, type, default: def, desc }) => (
            <div key={name} className="grid grid-cols-5 gap-4 py-4">
              <Text as="code" variant="mono">{name}</Text>
              <Text as="code" variant="mono" className="col-span-2 text-wrap">{type}</Text>
              <Text as="code" variant="mono">{def}</Text>
              <Text variant="caption">{desc}</Text>
            </div>
          ))}
        </Stack>
