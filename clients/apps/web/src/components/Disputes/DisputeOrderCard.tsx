...
export const DisputeOrderCard = ({ order }: { order: schemas['Order'] }) => (
  <ShadowBox className="flex flex-col gap-4">
    ...
    <div className="flex flex-col">
      ...
      <DetailRow label="Status" value={<OrderStatus status={order.status} />} />
      <DetailRow
        label="Subtotal"
      ...
      />
      <DetailRow label="Tax" value={format(order.tax_amount, order.currency)} />
      <DetailRow
        label="Total"
      ...
      {order.applied_balance_amount !== 0 && (
        <>
          <DetailRow
            ...
            value={format(order.applied_balance_amount, order.currency)}
          />
          <DetailRow
            label="To be paid"
            value={format(order.due_amount, order.currency)}
          />
        </>
      )}
      <DetailRow
