...
export const PublishSettings = ({ article }: PublishModalContentProps) => {
  ...
  const handleArchiveArticle = useCallback(async () => {
    ...
    router.push(`/dashboard/${article.organization.slug}/posts`)
    hideArchiveModal()
  ...
  const onSubmit = async () => {
    ...
    try {
      ...
      if (slugChanged) {
        router.push(
          `/dashboard/${article.organization.slug}/posts/${updatedArticle.slug}#settings`,
        )
      }
    } catch (e) {
    ...
    <FormField
      ...
      render={({ field }) => {
        const onSaveSlug = async () => {
          ...
          // Redirect
          router.push(
            `/dashboard/${art.organization.slug}/posts/${art.slug}#settings`,
          )
