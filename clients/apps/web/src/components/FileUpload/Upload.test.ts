...
describe('Upload', () => {
  ...
  describe('create', () => {
    it('uses the file MIME type when the browser provides one', async () => {
      const upload = createUpload()

      vi.spyOn(upload, 'getMultiparts').mockResolvedValue({
        sha256base64: 'sha256',
        parts: [],
      })
      ...
      expect(api.POST).toHaveBeenCalledWith('/v1/files/', {
    ...
    it('infers the MIME type from the file extension when the browser omits it', async () => {
      const upload = createUpload({
        service: 'support_case_attachment',
        file: new File(['test'], 'image.jpg', { type: '' }),
      })
    ...
    it('falls back to application/octet-stream for unknown extensions', async () => {
      const upload = createUpload({
        file: new File(['test'], 'archive.bin', { type: '' }),
      })
