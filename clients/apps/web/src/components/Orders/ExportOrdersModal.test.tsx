...
const { default: ExportOrdersModal } = await import('./ExportOrdersModal')

const organization = {
  id: 'org-1',
  created_at: '2020-01-01T00:00:00Z',
} as Parameters<typeof ExportOrdersModal>[0]['organization']
...
describe('ExportOrdersModal', () => {
  ...
  it('does not shift boundaries when UTC timezone is selected (negative offset, UTC-4)', () => {
    ...
    // Regression guard: `toUTCBoundary` would have moved the boundary to
    // 2024-06-15T00:00:00Z (4h earlier), incorrectly including orders from the
    // previous evening.
  ...
  it('does not shift boundaries when UTC timezone is selected (positive offset, UTC+9)', () => {
    ...
    // 2024-06-15T00:00:00Z (9h later), incorrectly excluding orders from the
    // early morning of the selected day.
    expect(url.searchParams.get('created_after')).toBe(
