...
export const OverlayContents = (props: {
  onClose: () => void
  issue: IssueDashboardRead
  issueOrg: OrganizationPublicRead
  issueRepo: RepositoryRead
  onSelectOrg: (selected: string) => void
  selectedOrg?: OrganizationPrivateRead
  ...
  pledge: any
  isDone: boolean
  onClickPledge: (e: React.MouseEvent<HTMLButtonElement>) => void
  isSyncing: boolean
    ...
    >
      <div className="z-0 block flex h-full w-full flex-row overflow-hidden rounded-2xl bg-white shadow-2xl">
        ...
        <div className="flex min-h-full flex-1 flex-col space-y-3 p-5 text-black/80">
          ...
          {isDone && (
            <span className="text-center text-lg">Thanks for pledging! 🎉</span>
          )}
        </div>
      </div>
    </div>
