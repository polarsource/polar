...
> = ({ index, currency }) => {
  ...
  const currentPricingType = watch(`prices.${index}.seat_tiers.pricing_type`)
  ...
  const handleTieringModelChange = useCallback(
    (value: TieringModel) => {
      ...
      if (value === 'fixed') {
        setValue(
          `prices.${index}.seat_tiers.pricing_type`,
          'volume' as schemas['SeatPricingType'],
        )
        const currentTiers = getValues(`prices.${index}.seat_tiers.tiers`)
        ...
        setValue(
          `prices.${index}.seat_tiers.pricing_type`,
          'graduated' as schemas['SeatPricingType'],
        )
      } else {
        setValue(
          `prices.${index}.seat_tiers.pricing_type`,
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      {/* Hidden field to keep pricing_type in sync */}
      <FormField
        control={control}
        name={`prices.${index}.seat_tiers.pricing_type` as const}
        render={({ field }) => (
