...
const SubNav = (props: { items: (SubRoute & { active: boolean })[] }) => {
  return (
    <div className="flex flex-row items-center gap-x-2 rounded-xl bg-gray-100 p-1 dark:border-gray-800 dark:bg-gray-900">
      {props.items.map((item) => {
        const className = classNames(
          item.active
            ? 'bg-white dark:bg-gray-950 shadow-md text-gray-950 dark:text-gray-300 font-medium'
...
const DashboardTopbar = ({
  ...
  const className = classNames(
    ...
    'flex h-20 w-full items-center justify-between space-x-4 bg-white dark:bg-gray-950 border-b border-gray-200 dark:border-gray-800',
  )
