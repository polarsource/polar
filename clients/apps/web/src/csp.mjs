...
const withLoopbackTwin = (value) => {
  if (ENVIRONMENT !== 'development' || !value) return value || ''
  if (value.includes('127.0.0.1')) {
    return `${value} ${value.replaceAll('127.0.0.1', 'localhost')}`
  }
  if (value.includes('localhost')) {
    return `${value} ${value.replaceAll('localhost', '127.0.0.1')}`
  }
  return value
}

const API_ORIGINS = withLoopbackTwin(process.env.NEXT_PUBLIC_API_URL)
const S3_UPLOAD_ORIGINS = withLoopbackTwin(process.env.S3_UPLOAD_ORIGINS)
...
const baseCSP = () => `
    default-src 'self';
    connect-src 'self' ${API_ORIGINS} ${S3_UPLOAD_ORIGINS} https://api.stripe.com https://maps.googleapis.com https://*.google-analytics.com https://chat.uk.plain.com https://prod-uk-services-attachm-attachmentsuploadbucket2-1l2e4906o2asm.s3.eu-west-2.amazonaws.com;
...
export const nonEmbeddedCSP = () =>
  flatten(`
  ${baseCSP()}
  form-action 'self' ${API_ORIGINS} polar:;
