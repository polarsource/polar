  ...
  return (
    <main className="flex flex-col items-center gap-y-16 py-16">
      ...
      <div className="flex w-full max-w-2xl flex-col gap-y-6">
        <div className="flex flex-col gap-y-6 rounded-3xl bg-white p-8">
          ...
          <div className="flex flex-row gap-x-4">
            <input
              className="w-full rounded-full bg-slate-100 px-4 text-sm placeholder-slate-400"
              type="email"
              placeholder="Enter your email..."
            />
            <button className="rounded-full border-none bg-blue-600 px-4 py-2 text-sm text-white">
              Subscribe
            </button>
          </div>
        </div>
        <div className="grid grid-cols-1 gap-6 md:grid-cols-3">
          {subscriptionTiers.items
            ...
            .map((tier) => (
              <div
                key={tier.id}
                className="flex w-full flex-col gap-y-6 rounded-3xl bg-white p-6"
              >
                ...
                <p className="h-full text-sm text-slate-500">
                  {tier.description}
                </p>
                <button className="self-start rounded-full border-none bg-blue-600 px-3 py-1.5 text-xs text-white">
                  Subscribe
                </button>
              </div>
            ))}
