 ...
 */
type UploadFilterFunction<
  TEntry extends AstroCollectionEntry,
  TArticle extends PolarArticle,
> = (data: UploadBuilderFunctionData<TEntry, TArticle>) => boolean
...
> implements PromiseLike<PolarUploadResult>
{
  ...
  private async getArticles(): Promise<
    PolarResult<Article[], PolarUploadError>
  > {
  ...
  public async then<TResult1 = PolarUploadResult, TResult2 = never>(
    ...
    const toProcess: UploadBuilderFunctionData<TEntry, PolarArticle>[] =
    ...
    const toCreate = processed.filter(
      (article) => !articles.find((a) => a.slug === article.slug),
    )
    ...
    const createResults = await Promise.all(
      toCreate.map((article) => this.createArticle(article)),
    )
