 ...
 */
import { type Polar as PolarAPI } from '@polar-sh/sdk';
import { HTTPValidationError, SDKError } from '@polar-sh/sdk/models/errors';
import type { ArticleCreate, ArticleUpdate, Article } from '@polar-sh/sdk/models/components';
...
> implements PromiseLike<PolarUploadResult>
{
	...
	private convertError(error: unknown): PolarUploadError {
		if (error instanceof SDKError) {
			return new PolarUploadError(error.message, error.statusCode, {
				cause: error,
			});
		}
		if (error instanceof HTTPValidationError) {
			return new PolarUploadError(error.message, 422, {
				cause: error,
			});
		}
		if (error instanceof Error) {
			return new PolarUploadError(error.message, 500, {
				cause: error,
			});
		}
		return new PolarUploadError('An unknown error occurred.', 500, {
			cause: error,
		});
	}
	...
	private async getArticles(): Promise<PolarResult<Article[], PolarUploadError>> {
		...
		try {
			const response = await this.client.articles.list({
				...
				visibility: [
					'public',
					...(this.options.showUnpublished ? ['hidden' as const, 'private' as const] : []),
				],
				page,
				limit: 100,
			});
			...
			for await (const page of response) {
				articles.push(...page.result.items);
			}
		} catch (error) {
			return {
				data: null,
	...
	private async createArticle(
		...
		try {
			const response = await this.client.articles.create(article);
	...
	public async then<TResult1 = PolarUploadResult, TResult2 = never>(
		...
		const createResults = await Promise.all(
			toCreate.map((article) =>
				this.createArticle({ ...article, organization_id: this.options.organizationId })
			)
		);
