...
export const priceOf = (p: PerUnit, quantity: bigint): Money => mul(p.price, quantity);
...
class PlanBuilder {
  ...
  meter<S extends MeterState>(name: string, aggregation: Aggregation<S>, price: PerUnit): this {
    ...
    this.lines.push({ name, aggregation: aggregation as unknown as Aggregation<MeterState>, price });
    return this;
  }
  ...
  build(): Plan {
    ...
    const lines = [...this.lines].sort((a, b) => (a.name < b.name ? -1 : a.name > b.name ? 1 : 0));
