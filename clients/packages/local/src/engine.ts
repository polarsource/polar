...
export const applyEvent = (
  ...
  const prev = meters.get(line.name) ?? {
    state: line.aggregation.init,
    sourceEventIds: [],
  }
...
export const replay = (
  ...
  from = emptyState,
): EngineState => events.reduce((s, e) => applyEvent(plan, s, e), from)
...
export const invoice = (
  ...
  const lines: LineItem[] = plan.lines.map((line) => {
    const acc = meters.get(line.name) ?? {
      state: line.aggregation.init,
      sourceEventIds: [],
    }
...
export interface Snapshot {
  readonly upToSeq: number
  readonly byCustomer: Record<
    string,
    Record<string, { state: string | null; sourceEventIds: readonly string[] }>
  >
}
...
export const snapshot = (state: EngineState): Snapshot => {
  ...
  for (const customerId of [...state.byCustomer.keys()].sort()) {
    ...
    const out: Record<
      string,
      { state: string | null; sourceEventIds: readonly string[] }
    > = {}
    for (const meter of [...meters.keys()].sort()) {
      const acc = meters.get(meter)!
      out[meter] = {
        state: acc.state === null ? null : acc.state.toString(),
        sourceEventIds: acc.sourceEventIds,
      }
    }
...
export const restore = (snap: Snapshot): EngineState => {
  ...
  for (const [customerId, meters] of Object.entries(snap.byCustomer)) {
    ...
    for (const [meter, acc] of Object.entries(meters)) {
      m.set(meter, {
        state: acc.state === null ? null : BigInt(acc.state),
        sourceEventIds: acc.sourceEventIds,
      })
    }
