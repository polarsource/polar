      ...
      response.ok && !error
        ...
        : Effect.fail(
            new PolarApiError({
              ...
              message: messageOf(error, response.status),
            }),
          ),
    ),
  )
...
export const syncMeters = (
  ...
  Effect.gen(function* () {
    ...
    for (const m of yield* listMeters(client, options.organizationId))
    ...
    for (const meter of meters) {
      ...
      if (!current) {
        ...
        yield* runCall(() =>
          client.POST('/v1/meters/', {
            body: meter as unknown as schemas['MeterCreate'],
          }),
        )
