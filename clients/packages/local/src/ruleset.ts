...
export const rulesetHistory = (rulesets: readonly Ruleset[]): RulesetHistory => {
  const sorted = [...rulesets].sort((a, b) => a.effectiveFrom - b.effectiveFrom);
  const versions = new Set<string>();
  for (let i = 0; i < sorted.length; i++) {
    const r = sorted[i]!;
    if (versions.has(r.version)) throw new Error(`duplicate ruleset version "${r.version}"`);
    versions.add(r.version);
    if (i > 0 && sorted[i - 1]!.effectiveFrom === r.effectiveFrom) {
      throw new Error(`two rulesets share effectiveFrom ${r.effectiveFrom} ("${sorted[i - 1]!.version}", "${r.version}")`);
    }
  }
...
export const rulesetAt = (history: RulesetHistory, timestamp: number): Ruleset | undefined => {
...
export const rulesetByVersion = (history: RulesetHistory, version: string): Ruleset | undefined =>
  history.rulesets.find((r) => r.version === version);
