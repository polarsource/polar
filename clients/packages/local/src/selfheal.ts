  ...
  Effect.gen(function* () {
    ...
    for (const line of args.report.lines) {
      if (line.status === 'missing-meter') {
        skipped.push({
          meter: line.meter,
          reason: 'meter not in Polar — run syncMeters (push) first',
        })
        ...
        if (line.delta !== null && line.delta > 0n) {
          ...
          skipped.push({
            meter: line.meter,
            reason:
              "Polar counted more than we delivered — re-flush won't help",
          })
        } else {
          skipped.push({
            meter: line.meter,
            reason: 'Polar total not comparable (non-integer)',
          })
        }
      }
    }
    ...
    const r = yield* deliver(store, sink, targets, {
      ...options,
      reason: 'rejected on self-heal re-send',
    })
  ...
  Effect.gen(function* () {
    ...
    const heal = yield* selfHeal(
      ...
      sink,
      {
        ...
        period: args.period,
        ...(args.cursorName ? { cursorName: args.cursorName } : {}),
      },
      options,
    )
