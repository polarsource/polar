...
const json = (body: unknown, status = 200): Response =>
  new Response(canonicalJson(body), { status, headers: { "Content-Type": "application/json" } });
...
const periodFromQuery = (params: URLSearchParams): { from: number; to: number } | null => {
...
export const makeHandler = (deps: ServiceDeps) => async (request: Request): Promise<Response> => {
  ...
  try {
    if (request.method === "POST" && pathname === "/v1/ingest") {
      ...
      const result = await Effect.runPromise(Effect.either(ingest(deps.store, body.events)));
      if (result._tag === "Left") return json({ error: result.left.reason, at: result.left.index }, 400);
    ...
    if (request.method === "GET" && pathname === "/v1/preview") {
      ...
      if (!customer) return json({ error: "missing ?customer (or ?polar_customer)" }, 400);
      if (!period) return json({ error: "missing/invalid ?from and ?to (ISO timestamps)" }, 400);
      const version = url.searchParams.get("version") ?? undefined;
      const invoice = invoiceFor(deps.history, deps.store.all(), { customer, period, ...(version ? { version } : {}) });
      return json(invoice);
    }

    if (request.method === "POST" && pathname === "/v1/reconcile") {
      if (!deps.polarClient) return json({ error: "reconcile not configured (no Polar token)" }, 501);
      const ruleset = currentRuleset(deps.history);
      if (!ruleset) return json({ error: "no ruleset defined" }, 400);
      const body = (await request.json()) as { customer?: string; polar_customer?: string; from?: string; to?: string };
      ...
      if (!customer || Number.isNaN(from) || Number.isNaN(to)) return json({ error: "need customer, from, to" }, 400);
      const report = await Effect.runPromise(
        reconcile(deps.polarClient, deps.store, ruleset.plan, { customer, period: { from, to } }),
      );
      return json(report);
    }
