...
export interface IngestionSink {
  send(events: readonly UsageEvent[]): Effect.Effect<SinkResult, ApiUnavailable | BatchRejected>;
}
...
interface HTTPValidationError {
  readonly detail?: ReadonlyArray<{ loc?: ReadonlyArray<string | number>; msg?: string }>;
}
...
const summarizeValidation = (error: unknown, fallback: string): string => {
  ...
  return detail.map((d) => `${(d.loc ?? []).join(".")}: ${d.msg ?? "invalid"}`).join("; ");
};
    ...
    send: (events) =>
      Effect.gen(function* () {
        ...
        for (let i = 0; i < mapped.length; i++) {
          ...
          if (problem) {
            return yield* Effect.fail(
              new BatchRejected({ status: 422, message: `event ${events[i]!.id}: ${problem}` }),
            );
          }
        }
        ...
        const { data, error, response } = yield* Effect.tryPromise({
          ...
          try: () => config.client.POST("/v1/events/ingest", { body: { events: mapped } as unknown as schemas["EventsIngest"] }),
          // A thrown request is a network failure → transient, keep the events.
          catch: (cause) => new ApiUnavailable({ status: "network", message: String(cause) }),
        });

        if (response.ok && data) {
          return { accepted: data.inserted ?? events.length, duplicates: data.duplicates ?? 0 };
        }
        ...
        if (response.status === 422) {
          return yield* Effect.fail(
            new BatchRejected({ status: 422, message: summarizeValidation(error, "validation error") }),
          );
        }
        ...
        return yield* Effect.fail(
          new ApiUnavailable({ status: response.status, message, ...(retryAfter !== undefined ? { retryAfter } : {}) }),
        );
      }),
  };
};
