...
describe('compileMeters', () => {
    ...
    expect(meters).toEqual([
      {
        ...
        filter: {
          conjunction: 'and',
          clauses: [{ property: 'name', operator: 'eq', value: 'requests' }],
        },
      ...
      },
      {
        ...
        filter: {
          conjunction: 'and',
          clauses: [{ property: 'name', operator: 'eq', value: 'seats' }],
        },
      ...
      },
      {
        ...
        filter: {
          conjunction: 'and',
          clauses: [{ property: 'name', operator: 'eq', value: 'tokens' }],
        },
  ...
  test('is deterministic — same Plan compiles byte-for-byte identically', () => {
    expect(canonicalJson(compileMeters(plan))).toBe(
      canonicalJson(compileMeters(plan)),
    )
  })
  ...
  test('warns (and skips) on lastOf — no Polar equivalent', () => {
    const p = product('p')
      .meter('gauge', lastOf('level'), perUnit(micros(1n)))
      .build()
  ...
  test("warns on meter names shorter than Polar's 3-char minimum", () => {
    const p = product('p')
      .meter('io', count(), perUnit(micros(1n)))
      .build()
