...
export type FakeRoute = (method: string, path: string, opts: { body?: any; params?: any }) => FakeReply;
...
export const fakePolarClient = (route: FakeRoute): { client: PolarClient; calls: FakeCall[] } => {
  ...
  const make =
    ...
    (path: string, opts: { body?: any; params?: any } = {}) => {
      ...
      const response = new Response(null, { status: r.status ?? 200, ...(r.headers ? { headers: r.headers } : {}) });
