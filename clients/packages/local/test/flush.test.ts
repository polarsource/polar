...
import { ApiUnavailable, BatchRejected, type IngestionSink, type SinkResult } from "../src/sink";
 ...
 */
function controllableSink(opts: { outageForFirst?: number; poison?: ReadonlySet<string> } = {}) {
  ...
  const sink: IngestionSink = {
    send: (events) =>
      Effect.suspend((): Effect.Effect<SinkResult, ApiUnavailable | BatchRejected> => {
        if (outageLeft > 0) {
          outageLeft -= 1;
          return Effect.fail(new ApiUnavailable({ status: 503, message: "service down" }));
        }
        const bad = events.find((e) => poison.has(e.external_id));
        if (bad) return Effect.fail(new BatchRejected({ status: 422, message: `invalid ${bad.external_id}` }));
...
describe("flush — happy path", () => {
  test("delivers every event once and advances the cursor to the end", async () => {
    ...
    expect(report).toMatchObject({ delivered: 3, deadLettered: 0, halted: false });
...
describe("flush — API outage (the fault-tolerance case)", () => {
  test("an outage halts the tick with NOTHING lost and the cursor unmoved", async () => {
    const store = new MemoryEventStore();
    seed(store, ["a", "b", "c"]);
    const { sink, received, recover } = controllableSink({ outageForFirst: 100 });
...
describe("flush — poison isolation", () => {
  test("a permanently-rejected event is dead-lettered without blocking the rest", async () => {
    ...
    expect(report).toMatchObject({ delivered: 2, deadLettered: 1, halted: false });
...
describe("flush — durability across a restart (SQLite)", () => {
  ...
  test("append is idempotent on external_id across calls", async () => {
    ...
    const a = store.append({ name: "m", external_customer_id: "acme", metadata: { amount: 1 }, timestamp: new Date(0).toISOString(), external_id: "x" });
    const b = store.append({ name: "m", external_customer_id: "acme", metadata: { amount: 999 }, timestamp: new Date(5).toISOString(), external_id: "x" });
