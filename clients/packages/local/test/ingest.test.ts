...
const raw: readonly RawEvent[] = [
  { name: "tokens", external_customer_id: "acme", metadata: { amount: 100 }, external_id: "a" },
  { name: "tokens", external_customer_id: "acme", metadata: { amount: 100 }, external_id: "a" }, // dup
  { name: "requests", external_customer_id: "acme", metadata: {}, external_id: "b" },
];
...
describe("ingestion (clock + durable append)", () => {
  ...
  test("a caller-supplied timestamp is preserved", async () => {
    ...
    const [event] = await runDeterministic(
      ingest(store, [{ name: "tokens", external_customer_id: "acme", metadata: { amount: 1 }, external_id: "z", timestamp: ts }]),
    );
  ...
  test("non-integer metadata is rejected at the door with IngestError", async () => {
    ...
    const exit = await runDeterministic(
      Effect.exit(
        ingest(store, [
          { name: "gpu", external_customer_id: "acme", metadata: { gb_seconds: 1.5 }, external_id: "bad" },
        ]),
      ),
    );
