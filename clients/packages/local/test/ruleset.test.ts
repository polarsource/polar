...
describe('rulesetHistory', () => {
  ...
  test('rejects duplicate versions and colliding effectiveFrom', () => {
    expect(() => rulesetHistory([v1, { ...v2, version: '1.0.0' }])).toThrow(
      'duplicate ruleset version',
    )
    expect(() =>
      rulesetHistory([v1, { ...v2, effectiveFrom: v1.effectiveFrom }]),
    ).toThrow('share effectiveFrom')
  })
...
describe('invoiceFor — what-if', () => {
  test('the same January usage under v2 yields the v2 price', () => {
    const whatIf = invoiceFor(history, events, {
      customer,
      period: jan,
      version: '2.0.0',
    })
  ...
  test('throws on an unknown version or a period before any ruleset', () => {
    expect(() =>
      invoiceFor(history, events, { customer, period: jan, version: '9.9.9' }),
    ).toThrow('version "9.9.9"')
    const ancient: Period = {
      from: Date.UTC(2025, 0, 1),
      to: Date.UTC(2025, 1, 1),
    }
    expect(() =>
      invoiceFor(history, events, { customer, period: ancient }),
    ).toThrow('no ruleset effective')
  })
})
