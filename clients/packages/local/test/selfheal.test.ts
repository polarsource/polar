    ...
    return run(
      Effect.gen(function* () {
        ...
        const heal = yield* selfHeal(
          store,
          sink,
          { report, customer, period },
          { retrySchedule: noRetry },
        )
...
describe('reconcileAndHeal — the closed loop', () => {
  ...
  function fakePolar(
    initial: Array<{ key: string; name: string; amount: number }>,
  ) {
  ...
  test('reconcile finds the gap, heal re-sends, second reconcile is clean', async () => {
    ...
    const { sink, client } = fakePolar([
      { key: 't1', name: 'tokens', amount: 100 },
    ])
    ...
    expect(result.before.lines[0]).toMatchObject({
      local: 150n,
      polar: 100n,
      delta: 50n,
    })
    ...
    expect(result.after.lines[0]).toMatchObject({
      local: 150n,
      polar: 150n,
      delta: 0n,
    })
  ...
  test('is a no-op when reconciliation is already clean', async () => {
    ...
    const result = await run(
      reconcileAndHeal(client, sink, store, plan, { customer, period }),
    )
