...
describe('service handler', () => {
  ...
  test('GET /v1/preview computes a local invoice from the ruleset', async () => {
    ...
    const res = await handler(
      get(
        '/v1/preview?customer=acme&from=2026-01-01T00:00:00Z&to=2026-02-01T00:00:00Z',
      ),
    )
    expect(res.status).toBe(200)
  ...
  test('GET /v1/preview validates its query params', async () => {
    ...
    expect(
      (
        await handler(
          get('/v1/preview?from=2026-01-01T00:00:00Z&to=2026-02-01T00:00:00Z'),
        )
      ).status,
    ).toBe(400)
