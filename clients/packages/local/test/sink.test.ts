...
const sentEvents = (calls: { opts: { body?: any } }[]) => calls[0]!.opts.body.events as PolarEvent[];
...
describe("polarSink — request mapping (forward the Polar-shaped event)", () => {
  test("forwards the event verbatim and strips the internal envelope", async () => {
    const { client, calls } = fakePolarClient(() => ({ data: { inserted: 1, duplicates: 0 } }));
  ...
  test("a customer_id (Polar UUID) event is forwarded as customer_id", async () => {
    ...
    await run(polarSink({ client }).send([event({ external_customer_id: undefined, customer_id: "cus_123" })]));
  ...
  test("a custom mapEvent overrides the default mapping", async () => {
    ...
    await run(
      polarSink({
        client,
        mapEvent: (e) => ({ name: "custom", external_customer_id: "x", external_id: e.external_id, metadata: {} }),
      }).send([event()]),
    );
    expect(sentEvents(calls)[0]).toMatchObject({ name: "custom", external_customer_id: "x" });
  });
});
...
describe("polarClient factory", () => {
  test("targets the right host per environment", () => {
    ...
    expect(polarClient({ token: "t", server: "sandbox" }).baseUrl).toBe("https://sandbox-api.polar.sh");
    expect(polarClient({ token: "t", baseUrl: "http://localhost:9999" }).baseUrl).toBe("http://localhost:9999");
  });
});

describe("polarSink — response & error classification", () => {
  test("200 returns inserted/duplicates from the response", async () => {
    const { client } = fakePolarClient(() => ({ data: { inserted: 3, duplicates: 2 } }));
    const exit = await run(polarSink({ client }).send([event(), event(), event()]));
    expect(exit._tag).toBe("Success");
    if (Exit.isSuccess(exit)) expect(exit.value).toEqual({ accepted: 3, duplicates: 2 });
  });

  test("422 → BatchRejected (permanent) with a parsed validation message", async () => {
    const detail = [{ loc: ["body", "events", 0, "name"], msg: "String should have at most 128 characters" }];
    const { client } = fakePolarClient(() => ({ status: 422, error: { detail } }));
  ...
  test("429 → ApiUnavailable (transient) carrying Retry-After", async () => {
    const { client } = fakePolarClient(() => ({ status: 429, headers: { "Retry-After": "30" } }));
    const exit = await run(polarSink({ client }).send([event()]));
    if (Exit.isFailure(exit) && exit.cause._tag === "Fail" && exit.cause.error._tag === "ApiUnavailable") {
...
describe("polarSink — local pre-validation against Polar limits", () => {
  test("an over-long name is rejected locally without calling the API", async () => {
    const { client, calls } = fakePolarClient(() => ({ data: { inserted: 1 } }));
    const exit = await run(polarSink({ client }).send([event({ name: "x".repeat(129) })]));
