...
import { type ComponentPropsWithoutRef, type ElementType, type JSX, type ReactNode } from "react";
...
} & Omit<ComponentPropsWithoutRef<E>, keyof TextStyleProps | "className" | "loading">;
...
const renderSingleLineSkeleton = (placeholder: ReactNode): ReactNode => (
  <>
    ...
    <span aria-hidden className={SKELETON_CLASSES} style={{ position: "absolute", inset: 0 }} />
  </>
);
...
function Text<E extends TextTag = "p">({
  ...
  if (loading) {
    ...
    if (lines > 1) {
      ...
      content = renderSingleLineSkeleton(placeholderText ?? children ?? "Loading...");
  ...
  return (
    <Tag
      className={twMerge(textVariants({ variant, color, align, wrap }), className)}
      style={mergedStyle}
      {...(props as object)}
    >
      {content}
