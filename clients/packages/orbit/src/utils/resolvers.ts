...
import { borderRadii, breakpoints, colors, shadows, spacing } from "../tokens/tokens.stylex";
import {
  alignContentStyles,
  alignItemsStyles,
  alignSelfStyles,
...
// Friendly flex/grid alignment names map to their CSS keywords. The stylex
// stylesheets handle this for non-responsive values, but responsive/pseudo
// branches write CSS directly and need the mapped keyword.
const ALIGN_KEYWORD_MAP: Record<string, string> = {
  start: "flex-start",
  end: "flex-end",
...
function alignKeyword(v: string): string {
  return ALIGN_KEYWORD_MAP[v] ?? v;
}

const justifyContentValue = alignKeyword;
const alignContentValue = alignKeyword;
const alignItemsValue = alignKeyword;
const alignSelfValue = alignKeyword;
...
export function resolveBoxStyles(props: BoxStyleProps, scopeClass: string): ResolvedStyles {
  ...
  // --- Colors (token-based) ---
  addTokenProp(backgroundColorStyles, "background-color", props.backgroundColor, colorCss);
