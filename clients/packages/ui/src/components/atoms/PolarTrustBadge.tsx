...
const badgeVariants = cva(
  'flex items-center gap-x-2 rounded-full border transition-colors select-none w-fit',
  {
    variants: {
      theme: {
        ...
        glass:
          'bg-white/10 backdrop-blur-md border-white/20 text-white shadow-sm',
      },
      variant: {
        compact: 'h-8 px-3',
        full: 'h-10 px-4',
        inline: 'h-auto border-none bg-transparent px-0 rounded-none',
      },
    },
    defaultVariants: {
      theme: 'dark',
...
export interface PolarTrustBadgeProps extends VariantProps<
  typeof badgeVariants
> {
  className?: string
}

export const PolarTrustBadge = ({
  className,
  variant,
  theme,
}: PolarTrustBadgeProps) => {
  return (
    <div className={twMerge(badgeVariants({ variant, theme }), className)}>
      <svg
        width="20"
        height="20"
        viewBox="0 0 29 29"
