...
const SelectContent = ({
  ...
  React.useImperativeHandle(ref as React.Ref<HTMLDivElement>, () => contentRef.current as HTMLDivElement);
  React.useEffect(() => {
  ...
  React.useEffect(() => {
    ...
    const viewportEl = contentRef.current?.querySelector<HTMLElement>(
...
const SelectItem = ({
  ...
  <SelectPrimitive.Item
    ref={ref}
    className={cn(
      'rounded-xs focus:bg-accent focus:text-accent-foreground data-disabled:pointer-events-none data-disabled:opacity-50 relative flex w-full cursor-default select-none items-center py-1.5 pl-8 pr-2 text-sm outline-none',
      className,
    )}
    {...props}
  >
