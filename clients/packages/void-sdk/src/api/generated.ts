...
export const SubscriptionStatus = Schema.Literals(["incomplete", "incomplete_expired", "trialing", "active", "past_due", "canceled", "unpaid", "paused"]).annotate({ "title": "SubscriptionStatus", "identifier": "SubscriptionStatus" })
...
export type EventType = { readonly "name": string, readonly "source": EventSource, readonly "occurrences": number }
export const EventType = Schema.Struct({ "name": Schema.String.annotate({ "title": "Name" }), "source": EventSource, "occurrences": Schema.Number.annotate({ "title": "Occurrences" }).check(Schema.isInt().annotate({ "expected": "an integer" })) }).annotate({ "title": "EventType", "identifier": "EventType" })
...
export type EventsTypesParams = { readonly "external_identity_id"?: string | null }
export const EventsTypesParams = Schema.Struct({ "external_identity_id": Schema.optionalKey(Schema.Union([Schema.String, Schema.Null]).annotate({ "title": "External Identity Id", "description": "Only this actor" })) })
export type EventsTypes200 = ReadonlyArray<EventType>
export const EventsTypes200 = Schema.Array(EventType).annotate({ "title": "Response Events:Types" })
export type EventsTypes422 = HTTPValidationError
export const EventsTypes422 = HTTPValidationError
...
export const make = (
  ...
  return {
    ...
    "eventsTypes": (options) => HttpClientRequest.get(`/v1/void/events/types`).pipe(
    HttpClientRequest.setUrlParams({ "external_identity_id": options?.params?.["external_identity_id"] as any }),
    withResponse(options?.config)(HttpClientResponse.matchStatus({
      "2xx": decodeSuccess(EventsTypes200),
      "422": decodeError("EventsTypes422", EventsTypes422),
      orElse: unexpectedStatus
    }))
  ),
...
* **Scopes**: `void:read` `void:write`
*/
readonly "eventsTypes": <Config extends OperationConfig>(options: { readonly params?: typeof EventsTypesParams.Encoded | undefined; readonly config?: Config | undefined } | undefined) => Effect.Effect<WithOptionalResponse<typeof EventsTypes200.Type, Config>, HttpClientError.HttpClientError | SchemaError | VoidApiError<"EventsTypes422", typeof EventsTypes422.Type>>
  /**
* **Scopes**: `void:read` `void:write`
*/
...
export const makePromiseClient = (api: Client, run: Run) => ({
  ...
  "events": {
    "types": (params: NonNullable<NonNullable<Parameters<Client["eventsTypes"]>[0]>['params']> = {}) => run(api["eventsTypes"]({ params })),
