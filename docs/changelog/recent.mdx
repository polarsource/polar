...
Scheduled subscription changes — product swaps, price updates, seat-count changes — now appear both in the merchant dashboard and the customer portal. A clear "Scheduled Update" section shows what will change and when it applies, so there are no surprises at the next billing cycle.

<img class="border rounded" src="/assets/changelog/2026-04-22/pending-subscription-update.png" />

## Korean Checkout Localization

Checkout is now fully localized in Korean (`ko`). Merchants selling to Korean customers get a native buying experience end-to-end — product details, payment fields, and order summary all render in Korean.

<img class="border rounded" src="/assets/changelog/2026-04-22/korean-checkout.png" />
...
Dashboard metrics queries now sit behind a short-lived cache, reducing load on the analytics backend and making repeat loads of revenue, subscription, and product charts noticeably faster.

## Out-of-Order Event Ingestion

Events can now be ingested with a `pending_parent` relationship: child events no longer have to arrive after their parent. The relationship is resolved when the parent event lands, making usage-metering pipelines tolerant to out-of-order delivery.

## Anonymous Seat Assignment Requires Checkout Secret

Assigning a seat anonymously — e.g. right after purchase, before the buyer has logged in — now requires the checkout secret in addition to the checkout ID. The secret is already available in the purchase context, so this tightens the seat-invitation flow without disrupting the buy experience.

## Faster Orders Page Product Filter

The product filter on the orders page now loads in larger batches instead of fetching products one at a time. Filtering orders is significantly snappier for organizations with many products.

## Faster Checkout First Paint

Checkout pages now do more work server-side on initial render, reducing the client-side loading flash on first paint. Buyers see the full checkout UI faster after clicking a payment link.
</Update>
