...
## Benefits and discounts on the product overview

The product Overview tab now lists the benefits a product grants, so you can see what customers receive without opening the edit form. Discounts moved from a table into a paginated list, and the product's price now sits in the page header next to its status.

## Webhooks for discounts

Three new events fire when you create, update, or delete a discount: `discount.created`, `discount.updated`, and `discount.deleted`. Subscribe to them to keep your own pricing pages and caches in sync with the coupons you run, instead of polling the API.

## New `subscription.cycled` webhook

A new event fires the moment a subscription enters a new billing period, before the renewal order exists. It fires whether or not the renewal payment succeeds, so you no longer have to infer renewals from `order.created` and its `billing_reason`.

## Checkout adapts to the chosen payment method

Checkout now tracks which payment method the customer picks and adjusts the form to match. Methods that need a full billing address, such as UPI, reveal the complete address fields, and switching back to card hides them again. A missing address is caught at confirmation instead of failing later.

## Chargeback prevention explained on the order page

When Polar refunds an order on your behalf to head off a chargeback, the order page now shows a banner explaining why, alongside a "Chargeback Prevented" status. You no longer have to ask support why a refund you did not issue appeared.

## SSO creates accounts on first login

Signing in through your identity provider now creates the Polar account and the organization membership if they do not exist yet, with the member role. There are no invitations to send before someone's first login. Remove access in your identity provider and it is gone in Polar.
</Update>
...
Set an optional resume date when you pause and the subscription comes back on its own. This fits seasonal plans and account "freezes", where a customer steps away and returns later.

Drive it from the API through the [Update Subscription](/api-reference/subscriptions/update-subscription) endpoint, or let customers pause themselves from the Customer Portal by turning on **Enable subscription pause** under Settings → Customer portal. Two new webhook events, `subscription.paused` and `subscription.resumed`, tell you when the state changes.

[Read more](/features/subscriptions/manage#pause-and-resume)
</Update>

<Update label="2026-07-22">
## Single Sign-On

Connect your organization to your own identity provider over OpenID Connect, including Google Workspace, Okta, Microsoft Entra ID, and Keycloak. Your team signs in with the credentials they already use, and anyone who authenticates becomes a member of your organization, so there are no invitations to send. Enforce SSO to make your identity provider the only way in: revoke someone's access there and they lose access to Polar.

Single Sign-On is included in the Scale plan. Set it up under Settings → SSO.

[Read more](/features/sso)
</Update>

<Update label="2026-07-10">
## Added support for new local payment methods

We've enabled several additional local payment methods.

These payment methods are automatically displayed at checkout when they're available for a customer's location and purchase. No action is required from merchants.

Newly supported:

* 🇧🇪 Bancontact (EUR, one-time purchases)
* 🇵🇱 BLIK (EUR, one-time purchases)
* 🇦🇹 EPS (EUR, one-time purchases)
* 🇳🇱 iDEAL / Wero (EUR, one-time purchases)
...
[Read more](/features/checkout/session#ad-hoc-prices)
...
[Read more](/features/cost-insights/introduction)
...
[Read more](/api-reference/subscriptions/create-subscription)
...
<Update label="2025-10-17">
  ...
  Merchants can now [update existing subscriptions](https://polar.sh/docs/api-reference/subscriptions/update-subscription#subscriptionupdateproduct) from archived pricing schemes to current ones within the same product.
  ...
  A Return URL can now be set while generating a [checkout session](https://polar.sh/docs/api-reference/checkouts/create-checkout-session#body-return-url-one-of-0) or a [customer portal session](https://polar.sh/docs/api-reference/customer-sessions/create-customer-session#body-one-of-0-return-url-one-of-0). This allows you to preserve the context for the end users who visit either if they wish to go back to the application.

  ## Ability to search Customers via their External ID

  The [List Customers API](https://polar.sh/docs/api-reference/customers/list-customers) now accepts Customer's External ID [in the **query** parameter](https://polar.sh/docs/api-reference/customers/list-customers#parameter-one-of-0).
...
The [Customer State API](/api-reference/customers/get-customer-state) now properly handles customers with `trialing` subscriptions, so your integrations and dashboards always show an accurate subscription status.
...
This feature is both available through the [API](/api-reference/subscriptions/update-subscription) and the dashboard.
...
Maybe one of our neatest features to date! Customer State is a concept allowing you to query for the current state of a customer, including their **active subscriptions** and **granted [benefits](/features/benefits/introduction)**, in a single [API call](/api-reference/customers/get-customer-state-by-external-id) or single [webhook event](/api-reference/customerstate_changed).
