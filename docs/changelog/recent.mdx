...
Events can now be ingested with a `pending_parent` relationship, so child events no longer have to arrive after their parent. The relationship is resolved when the parent event lands, making usage metering pipelines tolerant to out of order delivery.
</Update>
