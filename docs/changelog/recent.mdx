...
Customers can now download a PDF receipt for any paid order from the customer portal, separate from the invoice. Receipts show the receipt number, payment method (card brand and last 4 digits), applied customer balance, and refunds. The link is also surfaced in transactional emails.

<img class="border rounded" src="/assets/changelog/2026-05-06/download-receipt-portal.png" />

## Refund Flow

The refund modal now previews the gross amount the customer will receive, refund plus tax, before you confirm. A new "Refund fully" shortcut fills the max in one click. The amount input is labeled exclusive of tax, and an inline note clarifies how processing fees behave on refunds.

<img class="border rounded" src="/assets/changelog/2026-05-06/refund-modal.png" />

## Cancel Pending Subscription Updates

Merchants and customers can now cancel a scheduled subscription update from the dashboard or the customer portal. Previously this required an API call. Builds on the pending subscription updates that shipped on 2026-04-22.

## Daily Payout Limit

Manual payouts are now capped at one withdrawal per 24 hours. The Finance withdraw modal shows the next available payout time when the cooldown is active.

## Trial MRR on the Subscriptions Dashboard

Two new tiles on the subscriptions dashboard track Trialing MRR and Trialing MRR Including Cancellations, separate from the headline MRR. See how much revenue sits in the trial pipeline at a glance.
</Update>
