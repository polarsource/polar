...
## Full Product Description on Checkout

Long product descriptions are no longer truncated on the checkout page. Buyers see the full pitch, formatting, and details right where they decide to buy, instead of a clipped preview with a "read more" they may never click.
</Update>

<Update label="2026-03-31">
## Streamlined Merchant Onboarding

A redesigned onboarding flow guides new merchants through account setup, organization details, and product creation step by step. Smarter validation, contextual error handling, and analytics on each step mean fewer drop-offs before the first sale.
...
## Korean Checkout, Portal, and Emails

Korean joins the list of fully translated languages across checkout, customer portal, and customer emails. Merchants selling to Korean buyers get a native end-to-end experience.
...
## Markdown Product Descriptions on Checkout

Product descriptions on checkout pages now render Markdown formatting, so headings, lists, links, and emphasis come through as intended instead of as raw text. Make your value proposition look the way you wrote it.
...
Three new webhook events have shipped: `member.created`, `member.updated`, and `member.deleted`. External systems can sync team membership changes in real time, perfect for provisioning seats in your own product as B2B teams grow or shrink.
...
## Delete Your Account from the Web Dashboard

Account deletion is now available from the web dashboard preferences, matching the mobile app. A clear Danger Zone section walks users through confirmation, surfaces any organizations they need to delete first, and signs them out cleanly when the deletion completes.
</Update>
