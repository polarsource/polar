...
Checkout is now fully localized in Korean (`ko`). Merchants selling to Korean customers get a native buying experience end to end, with product details, payment fields, and order summary all rendering in Korean.
...
Assigning a seat anonymously (for example, right after purchase and before the buyer has logged in) now requires the checkout secret in addition to the checkout ID. The secret is already available in the purchase context, so this tightens the seat invitation flow without disrupting the buy experience.
