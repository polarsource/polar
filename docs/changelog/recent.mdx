...
## Imported subscription webhook

A new `subscription.imported` webhook fires when Polar takes over billing of a subscription imported from another platform. The payload includes the Polar subscription plus `platform` and `external_id` so you can match it to the source subscription.

[Read more](/docs/integrate/webhooks/events)
</Update>
