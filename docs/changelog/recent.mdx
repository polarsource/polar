...
Polar identifies a customer by account, email address (ignoring plus-aliases), and payment card, so a fresh address on the same card doesn't buy a second run. A customer who enters an exhausted code is told on the code field, before the payment step.
