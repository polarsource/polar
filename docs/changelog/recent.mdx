...
## Seat Metrics Dashboard

A new Seats dashboard in Analytics tracks seat count, average seats per customer, and seat utilization rate alongside customer counts (active, new, churned). Merchants selling seat-based products can finally watch utilization trends and identify accounts that are over or under their plan without exporting data.

<img class="border rounded" src="/assets/changelog/2026-04-29/seat-metrics.png" />

## Mobile-Friendly Dashboard

The Analytics, Sales, Events, Checkouts, Subscriptions, and Webhook detail pages have been reworked for phone screens, with new stacked pickers for the metric view header, date range, and product filter. The dashboard is genuinely usable on mobile for the first time across these pages, so checking yesterday's revenue or a customer's subscription from your phone no longer requires zooming and horizontal scroll.

<img class="border rounded" src="/assets/changelog/2026-04-29/mobile-dashboard.png" />

## ARR, cARR, and Trial MRR Metrics

Three new revenue metrics joined the Analytics dashboards. Annual Recurring Revenue (ARR) and Committed ARR (cARR) project MRR over a yearly horizon, while Trial MRR shows the recurring revenue currently sitting in active trials. Useful for board reporting and watching how much pipeline is in trial conversion at any point in time.

## Discount Amount Shown on Metered Pricing

Checkout now displays the discount applied to usage-based prices in the pricing breakdown and on the unit-price label. Buyers on metered products can see exactly what their coupon is doing to per-unit cost before they pay.

## Custom Fields on Mobile Order Details

The Polar mobile app's order detail screen now displays any custom fields set on the order, matching the dashboard. Merchants reviewing orders on the go see the same buyer-supplied details as on the web.

## Georgian Tax ID Validation

Buyers can now enter a Georgian tax ID at checkout and have it validated. Merchants selling to Georgia-registered businesses can collect a verified tax ID for their invoices.
</Update>
