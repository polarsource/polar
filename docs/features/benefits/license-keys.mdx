...
If a key is exposed or compromised, merchants can rotate it from the benefit license keys page in the dashboard, or via `POST /v1/license-keys/{id}/rotate`. Only keys in `granted` or `disabled` status can be rotated; rotating a revoked key returns a `400` error.

Rotation generates a new key string on the same license key record. The previous key stops validating immediately. Status, usage, limits, expiry, and activations are preserved. The customer can copy the new key from the customer portal, or you can share it from the dashboard.
...
[`/v1/customer-portal/license-keys/validate`](/api-reference/customer_portal/validate-license-key) endpoint.
