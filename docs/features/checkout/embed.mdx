...
The same cause shows up as an error from the API rather than a blank frame, when a checkout link is opened from a host that isn't listed. The fix is the same.
