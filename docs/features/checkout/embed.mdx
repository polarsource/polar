...
List the hosts allowed to embed your checkout under **Settings → Preferences → Embedding**. Once the list is enforced, an embedded checkout only opens on a host you've listed.
...
<Note>
  Add every host in one go, staging and preview domains included — separate them with commas or spaces. Nothing breaks today, but a host you leave out will stop opening your checkout once we enforce the list.
</Note>
