...
A Checkout Link is a persistent URL with a preconfigured Checkout Session: products, discount, metadata, redirect URLs, and more. When a customer visits the link, Polar creates a fresh Checkout Session from that configuration and redirects the customer to it.
