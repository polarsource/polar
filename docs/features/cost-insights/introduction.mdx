...
1. **Ingest events with cost data**: Add a `_cost` property to events when ingesting them through Polar's Event Ingestion API
2. **Automatic aggregation**: Polar automatically aggregates costs alongside your revenue data
3. **Query and analyze**: Use the Metrics API to retrieve costs, profits, and customer LTV
...
<CardGroup cols={2}>
  ...
  <Card title="Cost Metrics" icon="chart-bar" href="/features/cost-insights/cost-metrics">
...
- Learn more about [Event Ingestion](/docs/events/ingestion)
- Explore the [Metrics API](/docs/api/metrics)
- Understand [Customer Analytics](/docs/features/customer-analytics)
