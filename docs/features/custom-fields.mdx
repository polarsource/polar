...
Click on **New Custom Field** to create a new one. You can also manage them programmatically using the [Custom Fields API](/api-reference/custom-fields/create-custom-field).
...
When [creating a Checkout Session](/api-reference/checkouts/create-checkout-session) from the API, you can also prefill the fields by setting the `custom_field_data` property, keyed by the **slug** of each field.
...
This data is also available from the [Orders](/api-reference/orders/get-order) and [Subscriptions](/api-reference/subscriptions/get-subscription) API, under the `custom_field_data` property. Each value is referenced by the **slug** of the field.
