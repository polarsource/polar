...
<Info>
**Several Months** defines the window during which invoices are eligible for the discount — not the length of service the discount covers. Any invoice billed within that many calendar months of the first redemption is discounted in full; invoices billed after are charged at full price. On a yearly plan this means a 1–12 month duration discounts only the first yearly invoice — multiply by 12 to cover additional years (e.g. `24` for 2 years).
</Info>
