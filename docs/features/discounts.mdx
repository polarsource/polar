...
<Info>
**Several Months** is counted in calendar months from when the discount is first applied — not in billing cycles. On a yearly plan, any duration from 1 to 12 covers only the first yearly invoice (in full, not prorated); to cover multiple years, multiply by 12 (e.g. `24` for 2 years).
</Info>
