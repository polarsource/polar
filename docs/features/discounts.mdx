...
Three events fire on the discount lifecycle: `discount.created`, `discount.updated` and `discount.deleted`. Each carries the full discount object.

Subscribe to them if you keep your own copy of your active offers, for example to render pricing pages without calling the API on every request. The events cover changes made from the dashboard as well as through the API, so a manual edit never leaves your cache stale.

See [Webhook Events](/integrate/webhooks/events) for the full list.
