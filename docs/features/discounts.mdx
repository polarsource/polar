...
- **Several Months** The discount is applied to any invoice billed within the given number of calendar months from the first redemption — not for that many months of service. On a yearly plan, a duration of 1–12 therefore catches only the first yearly invoice; multiply by 12 to cover additional years (e.g. `24` for 2 years).
