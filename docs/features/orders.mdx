...
| Status               | Meaning                                                                                   |
| -------------------- | ----------------------------------------------------------------------------------------- |
| `draft`              | The order is a draft, pending finalization. Invoice number is not yet assigned.           |
...
Draft orders are created via the off-session charges API and must be finalized before payment is attempted. Free orders — those with a total of zero, typically from a $0 subscription or a 100% discount — are marked `paid` immediately with no payment step.
