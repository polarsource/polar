...
## Hiding the seat counter on checkout

When creating a checkout session over the API for a seat-based product, the checkout page displays a seat counter that lets the customer pick how many seats they're buying. If you want to lock the seat count and hide the picker — for example, when you've already decided the quantity in your own UI — set `seats`, `min_seats`, and `max_seats` to the same value when creating the checkout session.
...
With the minimum and maximum pinned to the same number, the seat picker is hidden entirely — the customer doesn't see the seat quantity on the checkout page at all.
