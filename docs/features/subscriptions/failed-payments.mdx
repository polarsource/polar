...
The most reliable way for a customer to get back into `active` is to update their default payment method from the [Customer Portal](/features/customer-portal/introduction). Because Polar is PCI-compliant, card details never touch your servers — the portal handles that flow end to end. As soon as the payment method is updated, Polar retries the charge immediately rather than waiting for the next scheduled attempt.
...
If you're driving email or in-app messaging from your own system, these are the events that matter during recovery:

- [`subscription.updated`](/api-reference/webhooks/subscription.updated) — fired when the status changes, including `active → past_due` and `past_due → canceled`. This is the main signal to branch on.
- [`order.updated`](/api-reference/webhooks/order.updated) — fired when `next_payment_attempt_at` changes as retries are scheduled.
- [`order.paid`](/api-reference/webhooks/order.paid) — fired if a retry finally succeeds.
- [`subscription.revoked`](/api-reference/webhooks/subscription.revoked) — fired when retries are exhausted and the subscription is canceled.
