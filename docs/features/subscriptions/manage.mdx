...
- The subscription is always updated **immediately**. Only the invoicing is affected by the [proration behavior](/features/subscriptions/proration) you pick.
- The new product must share the subscription's currency and must match its "seat-based or not" shape — you can't switch a flat subscription to a seat-based product or vice versa.
...
For [seat-based subscriptions](/features/seat-based-pricing), update how many seats the customer is paying for. Like plan changes, the seat change is applied immediately and the billing impact is controlled by the proration behavior.
...
You can add, extend, or end a [trial](/features/trials) on any subscription:
...
Revoking ends the subscription **right now**: status moves to `canceled`, `ended_at` is set to the current time, and all benefits are revoked (license keys deactivated, Discord roles removed, repo access revoked, and so on). There's no refund — if you need to issue one, do it separately from [Refunds](/features/refunds).
...
Everything above is also available to customers through the [Customer Portal](/features/customer-portal/introduction), subject to your [portal settings](/features/customer-portal/settings):
...
Customers can't revoke immediately, change discounts, extend trials, or reschedule renewals — those are merchant-only actions.
