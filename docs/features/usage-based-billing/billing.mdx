...
<Note>
  By default, usage aggregation follows the billing interval. Products can optionally configure a <strong>meter interval</strong> (via `meter_interval`) to decouple metered concerns from the billing interval — for example, yearly billing with monthly meter resets and credit grants.
</Note>
