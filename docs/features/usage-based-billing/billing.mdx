...
Volume and graduated take the same tiers. Define one by an upper bound and a rate. The bound is inclusive, and each tier starts where the previous one ended. The last tier is always unbounded, since usage has no ceiling.
