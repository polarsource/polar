...
if (event.type === 'benefit_grant.created') {
  ...
  // `member` is the end user for both seats and direct purchases
  await grantAccess(grant.member.id, grant.member.email);
}
```

<Warning>
Do not branch on whether `member` is set to tell a seat grant from a direct
purchase. A direct purchase carries the buyer's own member, so `member` is
always set and the branch never fires. Read `grant.member.role` if you need to
tell the buyer from a seat holder, or `grant.member.customer_id` if you need the
billing customer.
</Warning>
