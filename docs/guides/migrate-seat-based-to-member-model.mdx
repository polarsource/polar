...
purchase: a direct purchase carries the buyer's own member, so presence tells you
nothing. Read `grant.member.role` if you need to tell the buyer from a seat
holder, or `grant.member.customer_id` if you need the billing customer.
</Warning>
