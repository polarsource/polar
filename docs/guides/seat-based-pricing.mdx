...
<Tip>
To hide the seat picker on the checkout page and lock the quantity (for example, when your own UI already collected the seat count), set `seats`, `min_seats`, and `max_seats` to the same value. With the bounds pinned, the customer can't change the seat quantity at checkout.
