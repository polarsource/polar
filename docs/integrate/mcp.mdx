...
Use the production server at `https://mcp.polar.sh/mcp/polar-mcp` for your live organization. While building or testing, use the sandbox server at `https://mcp.polar.sh/mcp/polar-sandbox` and give it a distinct name such as `polar-sandbox`.

Choose your client and add the appropriate server.
...
| List customer members by external ID | List the members of a customer identified by its external ID. |
...
| Get meter quantities | Get quantities of a meter over a time period. |
