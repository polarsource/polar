...
**Timing and Edge Case Considerations**

- The time between the initial renewal events (`subscription.updated`, `order.created`) and payment events (`order.updated`, `order.paid`) can vary depending on payment processing times
- If payment fails, additional webhook events may be triggered related to payment retry attempts
- For subscriptions with usage-based billing, the final amount in the order may differ from the base subscription price due to consumption charges from the previous period
- The `billing_reason` field in the order will be set to `subscription_cycle` to distinguish renewal orders from other order types
