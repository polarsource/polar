...
Each review is triggered based on:
* Reaching new transaction volume thresholds
* Changes in your risk profile or transaction patterns
* Routine compliance checks as required by payment networks

These reviews are typically completed quickly and help protect both you and your customers.
...
* Verify that you're a real business or creator with an established online presence
...
When transitioning customers from an unpaid (free) to a paid subscription or product, we may request a video demonstration to:

* Verify that paying customers receive the value they're paying for
* Ensure your product delivers on its promises and meets customer expectations
* Confirm there's a clear distinction and value upgrade between free and paid tiers
* Prevent disputes, refunds, and chargebacks from dissatisfied customers

This helps maintain trust on our platform and protects both you and your customers. A simple screen recording or demo showing the product's functionality is typically sufficient.

### How does changing the admin of an organization work?

To change the admin of an organization:

1. The current admin needs to invite the new admin as a member with admin privileges
2. Once the new admin accepts the invitation and has admin access, they can manage the organization settings
3. The original admin can then remove themselves if desired, as long as there's at least one other admin

**Important considerations:**
* Always ensure there's at least one active admin for the organization
* Verify the new admin's identity before granting access
* Review all organization settings and payment details during the transition
* Update any relevant billing or payout information if needed
