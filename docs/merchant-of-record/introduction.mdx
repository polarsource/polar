...
<AccordionGroup>
  <Accordion title="What is Polar's VAT number?">
    Polar is VAT-registered in multiple jurisdictions, and the number shown on your invoice depends on your billing address:

    - **EU customers**: `EU372061545`. This is an EU One Stop Shop (OSS) registration, which allows non-EU businesses like Polar (a Delaware C Corporation) to handle VAT for all EU countries through a single registration in Ireland.
    - **UK customers**: `GB458254961`. Polar's UK VAT registration, used for sales to customers in the United Kingdom.
    - **Customers elsewhere**: no VAT number is shown — Polar handles applicable sales taxes through other registrations as needed.
  </Accordion>
