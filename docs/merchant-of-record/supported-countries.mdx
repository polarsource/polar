...
<AccordionGroup>
  <Accordion title="Can I use Polar in countries (e.g. India) where Stripe is invite-only?">
    <Tip>Stripe Connect Express is a different product than the regular Stripe payments.</Tip>
    Yes. With Polar, you are able to receive payments in any currency from anywhere around the world and have them paid (i.e. perform a payout) in your local bank account in the local currency (for e.g. INR in case of a bank account in India) via our [Stripe Connect Express](https://docs.stripe.com/connect/express-accounts) integration.
  </Accordion>
