...
import { MigrationCoexistence } from "/snippets/migration-coexistence.jsx";
...
<Warning>
  **Never let the same subscription renew in both systems.** Use the switch in Polar to coordinate
  the change. Do not manually activate a matching Polar subscription while the Stripe subscription
  can still renew.
</Warning>
...
6. **Reconcile and clean up** — Confirm one owner per subscription, then retire old paths.
...
Polar validates the account and permissions before reading any billing data.
...
See **Restricted Stripe key permissions** under [Migration details](#migration-details)
...
```
Stripe account  →  Polar payment processor
     ↑                      ↑
  account owner         Polar verifies
  starts the copy       usable cards
```

Polar then checks which subscriptions have a usable card. Ask customers
without a copied card to add payment details again, leave them on Stripe, or
...
| Before switch            | After successful switch        |
| ------------------------ | ------------------------------ |
| Renews in Stripe         | Renews in Polar                |
| Paid period already paid | Same period kept               |
| Card verified in Polar   | Used on the next Polar renewal |
...
Compare the migration result with your own customer and access records. Once
every subscription has exactly one billing owner, remove old Stripe checkout
paths, subscription webhooks, scheduled billing jobs, and unused secrets.

**Done when:**

- New sales and moved subscriptions are managed in Polar
- Any exceptions are intentionally managed in Stripe
