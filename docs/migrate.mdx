...
Polar cancels each selected subscription on Stripe before activating it in
Polar. Do not manually activate a matching Polar subscription, as this can
...
4. **Move saved cards** — Stripe copies cards to Polar so renewals can succeed.
5. **Switch billing to Polar** — Selected subscriptions stop renewing on Stripe and continue on Polar.
6. **Reconcile and clean up** — Confirm each subscription bills in only one place, then retire old paths.
...
[customer state](/integrate/customer-state) into your application. New checkouts
...
Use a stable application customer ID as Polar's `external_customer_id` so you
can look customers up later. Keep Stripe IDs in your database for the
subscriptions that still renew there.
...
At the next renewal, Polar attempts to charge the copied card. If a payment
method is missing, the renewal fails and Polar emails the customer to add their
card again.
...
billing conflict. A subscription already scheduled to end also stays there
...
<AccordionGroup>
  <Accordion title="Restricted Stripe key permissions">
    ...
    stop billing on Stripe during the switch. All accounts read access lets
    Polar identify the Stripe account and reject unsupported Connect platforms.
