...
- For a **User**: all orgs they're a member of (via `UserOrganization`), optionally filtered by a required `permission`. Unscoped user sessions cannot reach organizations that enforce SSO (`sso_enforced`); those are only accessible through an SSO-scoped session. Token credentials (PAT/OAuth/OAT) are exempt from SSO enforcement.
