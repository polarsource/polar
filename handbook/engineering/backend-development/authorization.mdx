...
5. Returns an `AuthorizedOrganization` (or `AuthorizedAccount`, etc.) containing both the resource and the auth subject
...
async def get_account(
    ...
    # authorized.organization is already resolved and access-checked
    # authorized.auth_subject is the authenticated subject
...
async def do_thing(authorized: AuthorizeDoThing, ...) -> ...:
