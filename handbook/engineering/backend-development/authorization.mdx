...
For these, use `AuthorizeWebUser` from `server/polar/authz/dependencies.py`:
...
`AuthorizeWebUser` is the user-personal analogue of the always-allow `OrgPolicyGuard` variants. It returns `AuthSubject[User]` directly — same shape as the underlying authentication — since there's no resolved resource to wrap.
...
- `AuthorizeWebUser` — the authorization layer. Today it's policy-free, but it exists so future cross-cutting per-user checks (account suspended, identity not verified, MFA required, …) can be added in one place without touching endpoint signatures.

Endpoints rarely depend on `WebUserSession` directly — `AuthorizeWebUser` wraps it and is the public entry point.
