...
- For a **User**: all orgs they're a member of (via `UserOrganization`), intersected with `auth_subject.organization_ids` if the credential is down-scoped, optionally filtered by a required `permission`
