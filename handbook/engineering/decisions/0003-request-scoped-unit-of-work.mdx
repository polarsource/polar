...
that writes takes `AsyncSession`. A read that must observe a write from the same action
uses `AsyncSession` on the primary instead, and the handler says why. Replica lag on
that read would show a stale result.
