...
- `encrypt`/`decrypt` are async (a KMS call). Code that cannot await, such as authlib's flow,
  uses `encrypt_sync`/`decrypt_sync` instead. They block on the KMS call, so keep them to
  low-volume paths.
