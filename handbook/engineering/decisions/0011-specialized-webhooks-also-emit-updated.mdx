...
it as “anything changed on this resource.” Stripe sometimes sends a specialized event without
the generic updated one; Polar merchants who copied that pattern then miss the change.
...
`*.created` is a birth event, not an update. Firing `*.updated` after create is a
resource-specific compatibility choice (subscriptions do it), not required by this ADR.
...
- **Specialized event only**: fewer deliveries, but catch-all subscribers miss the change — the
  Stripe behavior we are explicitly not copying.
