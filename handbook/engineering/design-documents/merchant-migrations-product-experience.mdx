...
[Merchant Migrations to Polar](/engineering/design-documents/merchant-migrations). That document
...
### Connect and source recovery

**New.** A disconnected or expired source must not leave the detail page at a dead end. Keep the
migration and let an authorized merchant replace its restricted key in place:

```text
Stripe connection needs attention

Polar can no longer read this account. Add a new restricted key to continue.
No subscription has been changed.

[Replace Stripe key] [View required permissions]
```

The replacement endpoint validates account identity, live/test mode, and scopes before storing
the new encrypted credential. It refuses a key for a different Stripe account. If no irreversible
action has occurred, the merchant may also abandon and archive the migration without deleting its
audit history.
...
The default prepare action selects all eligible records with no unresolved decision, never
attention or unsupported records. Its banner states that selection crosses pagination:

```text
All 812 eligible subscriptions across 41 pages are selected.
7 needing a decision and 22 staying on Stripe are excluded.

[Review selection] [Prepare 812 subscriptions]
```

Changing tabs does not silently change the selection. Screen readers receive the same count and
scope through a live region.
...
The comparison is an explicit resolution screen:

```text
Compare subscriptions

Stripe                                  Polar
Pro · $19/month                         Pro · $19/month
Renews Sep 18                           Renews Sep 22
Active                                  Active

Both subscriptions are currently active and may bill this customer.

( ) Keep billing on Stripe; leave this record out of migration
( ) Keep billing on Polar; cancel Stripe outside this migration
( ) I need help resolving this customer

[Save decision]
```

Polar does not automatically cancel either subscription from this screen. The cutover engine's
duplicate-subscription guard remains in place.

The same check runs again at cutover because a customer can buy on Polar after assessment.

### Assessment: billing policy
...
**Change.** Place this information at the start of the Switch step, before transfer selection.
Show the webhook contract and offer a test delivery.
...
"Switch subscriptions" to "Review transfer".
...
The UI encourages a canary but does not invent one:

- If the merchant supplied test-customer IDs through the stable customer mapping, Polar can offer
  that exact set as a canary.
- Otherwise the merchant selects known internal customers from the table.
- Polar then filters out records with missing cards, unresolved mappings or tax, and renewals
  inside the safety window.

```text
Review first transfer

Your selected canary                                    10 subscriptions
2 selected records were excluded because they renew within 24 hours.
...
738 subscriptions bill on Polar
 82 subscriptions still bill on Stripe
  0 have unknown billing ownership

Stripe must remain connected for the 82 subscriptions listed in the receipt.
Their webhooks and customer portal routes must keep working.

[ ] I understand that some billing remains on Stripe.

[Export final receipt] [Close with billing on Stripe]
```

When no source-owned subscription remains, the acknowledgment and residual warning are omitted.
...
```text
PATCH /v1/merchant-migrations/{id}/source
POST  /v1/merchant-migrations/{id}/archive
```

Source replacement accepts a new credential but not a new account identity. Archive is allowed
only before source cancellation and retains the ledger for audit.
...
- Add source-key replacement on the existing migration detail page.
...
- State the exact cross-page scope of prepare selection and exclude unresolved records by
  default.
...
- Add optional webhook test-delivery evidence without presenting it as proof of correct
  entitlement logic.
- Add canary eligibility filtering and reuse merchant-supplied test-customer IDs when available.
