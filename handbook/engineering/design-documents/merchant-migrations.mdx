...
The merchant chooses per subscription at review, before Polar starts billing:

1. **Taxable, inclusive (default)** — the customer keeps paying the listed price. Polar takes VAT/sales tax out of it, so the merchant earns less. Pins `tax_behavior=inclusive` and `tax_exempted=false`.
2. **Taxable, exclusive** — tax is added on top. The customer pays more than the listed price. Pins `tax_behavior=exclusive`.

Inclusive is the default regardless of Stripe `automatic_tax`. Without a pin, Polar would fall back to the org's `default_tax_behavior`; a `location` default is exclusive for `CA`/`IN`/`US` and would bill those customers listed price plus tax. Pinning inclusive makes the absorb-tax outcome explicit, not an accident of the customer's country.

Stripe `automatic_tax` is shown on the review row as information only; it does not pick Polar's behavior.

The choice lives on the staged `CanonicalSubscription` (`canonical.tax_behavior` in the ledger JSONB) and is written onto Polar `Subscription.tax_behavior` at cutover. That overrides product/price and org defaults, so a later change to those defaults doesn't shift an already-migrated subscription. A precheck refresh keeps a merchant choice already stored on a pending row.

B2B reverse-charge is not a third tax option: Polar already derives it from `Customer.tax_id`. Carry the source VAT ID (`customer.tax_ids`, or `customer.tax_exempt = reverse`) onto `Customer.tax_id`; reverse charge then re-derives itself at the first Polar renewal. Miss it and the customer gets re-taxed as B2C. (Polar validates VAT ID format only, not VIES registry validity.)

`subscription.tax_exempted` is pinned `false` at import. Pre-registration grandfathering (keep a subscription untaxed) is not a review choice yet.

The change bites at the **first renewal on Polar**: inclusive lowers the merchant's net, exclusive raises the customer's charge (dunning/churn risk). US sales tax is typically exclusive; merchants who were adding tax on top should pick exclusive for those subscriptions.
...
| tax behavior (`default_tax_rates` / inclusive / none) | `subscription.tax_behavior` (+ `tax_exempted`) | merchant choice at review, default inclusive (see *Taxes*). `automatic_tax` is display-only. |
