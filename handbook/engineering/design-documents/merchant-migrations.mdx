...
The migration is split into 3 groups. The **setup** phase, where the merchant creates a new migration, checks that everything is OK, and creates the catalog on the Polar side. The **migration** phase, where we copy the PAN information from the current billing provider to Polar's account, and Polar moves each subscription one by one. The last phase is for the merchant to clean up.
...
Some payment methods can't be moved, like **wallet payments** or bank debits. In those cases we may need customers to re-enter their billing details (to discuss).

In this step, some actions are done by the merchant, their billing provider, Stripe, or us. We should show the current status and where we're pending information.

## Switching subscriptions

Once we have the catalog and payment methods, we should be able to start moving subscriptions.

Subscriptions keep their real status (`active`/`trialing`) but are held from billing with a `migration_billing_paused` flag, so the renewal scheduler skips them. The org must be ACTIVE. Before activating each subscription we run some checks to prevent double charging the customer:
...
| **Lemon Squeezy** | API key            | Not documented |                                 |
...
- **`MigrationRecord`** one row per imported thing (customer, product, subscription, order, …), mapping `source_id → target_id` with a status. We use this as an idempotency layer.
- **`SourceAdapter`** we will have one for every billing provider. Its main responsibility is to read the current billing provider API and provide a generic representation the migration service can work with.
- **`PanTransferStep`** the checklist for moving cards (step, owner, status — see Appendix B). Stored as a JSONB column on `MigrationJob`: a list of step objects.
- **`MigrationService`** the main orchestrator.
- **`CanonicalRecord`** to have everything normalized in memory first.
...
3. How do we trigger the cutover on live — polling, a manual action, or a Polar webhook? To check on a test.
4. What happens with subscriptions we can't migrate? Should we ping the team?
5. Should we store the old billing id (customer, subscription) in `user_metadata`?
...
- **Payments**
	- Warning: flag subscriptions with bank or wallet payments, where we may ask the customer to re-enter the details.
- **Organization**
	- Blocker: organization must be in a renewal-enabled status (REVIEW or ACTIVE).

## Appendix B: The PAN transfer process

Moving cards can take weeks, so it's tracked as a checklist. Each migration has one or more `PanTransferSteps` showing what's done, in progress, and pending. Each step has an **owner** (merchant, Polar Ops, Polar App, Stripe, or the billing provider) and a **status**. The steps depend on the transfer method:
...
- Payment Method Ids are not kept.
...
| 4   | Send the encrypted vault to Stripe over SFTP         | Billing provider          |
...
| Stripe                                                                   | Polar                                 | Notes                                 |
| ------------------------------------------------------------------------ | ------------------------------------- | ------------------------------------- |
| `cus_…`                                                                  | new `Customer` + `stripe_customer_id` | the PAN copy preserves the id `cus_…` |
| email / name / address / tax id                                          | `Customer` fields                     | should be unique per org              |
...
| `sub_…`                                                                  | `user_metadata`                       | Stripe marker (pending to decide)     |
| `current_period_end` / `billing_cycle_anchor`                            | period / anchor                       |                                       |
| `trial_*`, `cancel_at_period_end`, `discounts`, `default_payment_method` | same fields                           |                                       |
| Coupon                                                                   | `Discount`                            |                                       |
| `pm_…` (card)                                                            | `PaymentMethod`                       | copied via PAN copy                   |
| Link / wallet / SEPA / Bacs / ACH                                        | —                                     | can't move!                           |
| Charges / refunds / disputes, Stripe Tax history                         | —                                     | can't move!                           |

### Stripe-specific prechecks

On top of the generic ones in Appendix A. What's unique to Stripe:

- **Manual:** source account is a Connect platform (cards/subs may live on sub-accounts, not the platform).
- **Blocker:** India / RBI account — card data can't cross the border, so the PAN copy can't run.
- **Blocker:** tiered/graduated pricing (`billing_scheme=tiered`) can't be represented.
- **Blocker:** metered pricing (`usage_type=metered`) — to be implemented later.
...
- **Warning:** PMs that can't copy, like Link, wallet, SEPA, Bacs, ACH — customer needs to re-enter.
