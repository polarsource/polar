...
At review the merchant picks per subscription: **inclusive (default)** — customer pays the listed price, Polar takes tax out, pins `tax_behavior=inclusive` and `tax_exempted=false` — or **exclusive**, tax added on top. Inclusive is the default regardless of Stripe `automatic_tax` (info only). Without a pin, Polar falls back to the org `default_tax_behavior`; a `location` default is exclusive for `CA`/`IN`/`US`.
