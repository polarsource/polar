...
Some payment methods can't be moved, like **wallet payments**, Bacs, or legacy SEPA `source` objects — but cards, ACH, and SEPA `PaymentMethod`s do copy (see Appendix D). For the ones that can't, we may need customers to re-enter their billing details (to discuss).
...
- **`SourceAdapter`** we will have one for every billing provider (e.g. `StripeAdapter`). Its main responsibility is to read the current billing provider API and provide a generic representation the migration service can work with.
...
- **Payments**
	- Warning: flag subscriptions with wallet, Bacs, or legacy SEPA `source` payments, where we may ask the customer to re-enter the details (cards, ACH, and SEPA `PaymentMethod`s copy fine).
- **Organization**
