...
At review the merchant picks per subscription: **inclusive** — customer pays the listed price, Polar takes tax out — or **exclusive**, tax added on top. The default is computed from the source, then pinned (`tax_exempted=false`) so a later org `default_tax_behavior` / `location` change cannot move it:
