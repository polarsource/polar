...
| `items.data[].current_period_end`, `billing_cycle_anchor` | `current_period_end`, `anchor_day` | period moved onto the subscription **item** (API `2025-03-31.basil`); read it per item. `anchor_day` is day-of-month, so reduce the anchor timestamp. |
| `trial_*`, `cancel_at_period_end`, `default_payment_method` | same fields | |
| `discounts[]` (Coupon) | single `Discount` | Polar holds **one** discount per sub; Stripe dropped the singular `discount`, so read `discounts[]` and flag if there's more than one. |
...
| Link / wallets / Bacs / legacy SEPA `source` | — | not copyable → customer re-enters (Link/wallets: to confirm with Stripe). |
...
- **Warning:** PMs that can't copy: Link, wallets, Bacs, legacy SEPA `source` objects — customer re-enters. (Cards, ACH, and SEPA `PaymentMethod`s copy fine; Link/wallets to confirm with Stripe.)
