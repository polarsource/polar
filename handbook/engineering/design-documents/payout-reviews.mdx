...
In the frontend, `PayoutStatus.tsx` maps the new `held` value to the same "Pending" label and amber color as `pending`. The merchant sees one status, not two.

Payout endpoints are private and not part of the public SDK, so the new `held` status is not visible to integrators either. The hold is fully internal: neither the dashboard nor the API exposes it as distinct from a normal pending payout.
...
For the initial rollout we gate `PayoutService.create` behind a per-org flag in `Organization.feature_settings` (the same mechanism as `account_review_v2_enabled`). This lets us enable held payouts for one or two test orgs on prod first, then turn it on for everyone and remove the flag. Only `create` is gated. The release and cancel jobs are never gated, so any held payout that already exists always drains.
