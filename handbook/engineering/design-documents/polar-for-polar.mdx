...
Org roles map to member roles so that billing permissions align: `owner` and `admin` become `billing_manager`, `member` stays `member`. This ensures org owners/admins can manage billing details in the customer portal.
...
5. Receive Polar Webhooks — Add /integrations/polar/webhook endpoint with signature verification, plus a polar ExternalEventSource enum value and handlers for benefit_grant.* and order.created events.
