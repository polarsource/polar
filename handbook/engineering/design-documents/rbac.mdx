...
Replacing the body of that helper with a permission check flips all three at once. The **Finance** policies switch to the corresponding role-permission check too. The `finance` role receives `finance:read`, while `finance:manage` remains limited to `owner` and `admin`. Billing-info edits on the org's `Account` (legal name, address, etc.) piggyback on `organization:manage`, which is admin-only by role-permission assignment.
...
A net-new endpoint changes a member's role: `PATCH /v1/organizations/{id}/members/{user_id}` with a `role` body, guarded by the `members:set_role` permission. `admin`, `finance`, and `member` are accepted in the body; transitions to or from `owner` are rejected by this endpoint and flow exclusively through the ownership-transfer path. The members listing response gains a `role` field.

## Frontend

The dashboard's auth context gains role and permission information per organization, populated from the existing per-user organizations endpoint. A permission hook answers `hasPermission(perm, org)` from that server-provided permission set. The backend remains the source of truth on every request.
...
The dashboard reads the role and resolved permissions returned by the backend, gates UI controls on the permission helper, and displays policy denial messages from the backend. Dashboard routes remain visible in navigation; server layouts render a restricted state when the user lacks the required permission.
