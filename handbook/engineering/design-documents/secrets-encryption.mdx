...
We store most secrets in plain text in the database: OAuth client secrets and Slack bot and signing secrets. Anyone who reads a copy of the database can use them right away. OAuthAccount tokens are mid-migration: encrypted columns exist alongside the plain ones, and writes populate both (dual-write). This RFC encrypts the remaining secrets before we store them.
...
| Model | Column | Encrypted? | File |
|---|---|---|---|
| `OAuthAccount` | `access_token`, `refresh_token` | Dual-write (step 1 done) — use `set_tokens()` / `get_access_token()` / `get_refresh_token()` | `server/polar/models/user.py` |
| `SlackApp` | `client_secret`, `signing_secret`, `bot_token` | No | `server/polar/models/slack_app.py:30` |
| `OAuth2Client` | `client_secret`, `registration_access_token` | No | `server/polar/models/oauth2_client.py:20` |
