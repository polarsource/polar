...
KMS rotates neither. Automatic rotation covers only symmetric encryption keys whose material AWS generates; asymmetric and HMAC keys are excluded and must be rotated by creating a new key and moving traffic to it. KMS's role also differs from the rest of this document: for a signing key it holds the private key and performs the signature itself, rather than wrapping a key we hold.
