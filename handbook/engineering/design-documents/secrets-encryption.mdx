...
same dual-write → backfill → cut-over → drop. Registration runs inside authlib's synchronous
flow, so it writes both copies through `encrypt_sync`, and the RFC 7592 endpoints read the
ciphertext back through `decrypt_sync`.
