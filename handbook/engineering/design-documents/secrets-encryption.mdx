...
## Signing keys are a different category

The rotation above concerns keys that protect stored data. A signing key is not that: it produces tokens other parties verify, so rotating it is a publication problem, not a re-encryption problem. We hold two — the JWKS that signs OAuth2 id_tokens and the SSO `private_key_jwt` assertions, and the symmetric `SECRET` that keys the HMAC of stored token hashes and signs short-lived JWTs.

KMS rotates neither. Automatic rotation covers only symmetric encryption keys whose material AWS generates; asymmetric and HMAC keys are excluded and must be rotated by creating a new key and moving traffic to it. KMS's role here is the same as everywhere else in this document: it wraps the key, it does not own its lifecycle.

For the JWKS, half of that shape already exists. The document holds a set, each key carries a `kid`, `CURRENT_JWK_KID` names the one we sign with, and `/.well-known/jwks.json` publishes them all. Rotation is therefore: publish the new key, move the pointer, and retire the old one once nothing it signed is still in flight. We keep a retired public key published for 7 days — well past the 3600s id_token lifetime, and wide enough for the third-party identity providers that cache our public set for SSO.

The material itself stops travelling as plaintext: the document is stored as envelope ciphertext and decrypted once at process start. Local development and CI keep the plaintext file and make no KMS call, exactly as for the columns above. ADR-0010 fixes this shape; the step-by-step procedure is an on-call runbook.
...
1. Ask KMS for a data key:
   `GenerateDataKey(KeyId, EncryptionContext={"table": "slack_apps", "column": "bot_token", "id": "<row uuid>"})`.
...
2. Ask KMS to unwrap the data key:
   `Decrypt(wrapped data key, EncryptionContext={"table": "slack_apps", "column": "bot_token", "id": "<row uuid>"})`.
...
The backend assumes an IAM role via Render's OIDC federation (`AssumeRoleWithWebIdentity`) — no long-lived AWS keys. The role gets only two actions on the one key, optionally pinned to an encryption context:
...
Config adds `AWS_KMS_KEY_ID` (the key id) and `ENCRYPTION_LOCAL_KEY` (the static key for local/CI) to `server/polar/config.py`. We provision one key per environment in Terraform and pass `POLAR_AWS_KMS_KEY_ID` and `AWS_ROLE_ARN` through a dedicated `secrets_kms` env group. Setting `AWS_ROLE_ARN` makes Render inject a web-identity token, so the boto3 client assumes the role via OIDC with no static keys.
