...
This proposal is to migrate this workflow to instead use the equivalent priorities as SQS queues and a Lambda function per queue. This will allow us to scale the concurrency of each task worker independently of each other. It will allow us
to automatically increase our capacity if we need to catch up on a queue we are lagging behind on, as well as allow us to easily split out work into new queues if we need to scale parts more independently.
...
flowchart LR
     ...
     producer --> sqs_b["SQS: polar-tasks-high-priority"]
     producer --> sqs_c["SQS: polar-tasks-medium-priority"]
     producer --> sqs_more["..."]

     sqs_a --> Lambda_a["Lambda: high_priority"]
     sqs_b --> Lambda_b["Lambda: medium_priority"]
     sqs_c --> Lambda_c["Lambda: low_priority"]
     ...
     Lambda_a -.->|"enqueue high_priority"| producer
     Lambda_c -.->|"enqueue medium_priority"| producer
