...
- The DB credentials live in a shared env group, so services may **not** redeploy automatically when Terraform updates it. For each affected service (API, workers, and pgBouncer), trigger **Manual Deploy → Deploy latest reference** in the Render Dashboard.
- Verify the API, workers, and pgBouncer come back up on the new credentials.
...
- [ ] API, workers, and pgBouncer healthy on the new credential
...
- pgBouncer services use `DB_USER` and `DB_PASSWORD`, also derived from the same source
