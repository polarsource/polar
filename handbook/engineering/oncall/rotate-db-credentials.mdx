...
In production and test, our terraform config populates env vars based on values read from Render:
...
Sandbox uses a dedicated `polar_sandbox` role set via Terraform variables instead, so this rotation does not change what sandbox services connect as.
