...
The tail query counts what migrated. These never appear in it, and they stop working:

- **Dormant OAuth2 integrations.** Refresh tokens do not expire and the row holds no plaintext,
  so nothing migrates one nobody uses. They go back through the authorization flow.
- **Personal access tokens.** There is no creation path, so the ones never used again cannot be
  reissued.
- **Backup codes.** Only the code someone enters gets rewritten.

Settle these before the first retirement, not before the first rotation.
