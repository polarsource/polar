...
export const iterListCheckoutLinks = (client: ClientBase) => {
  return async function* (
    ...
    while (true) {
      const response = await listCheckoutLinks(client)({ ...query, page, limit }, requestOptions);
...
export const getCheckoutLinks = (client: ClientBase) => {
   ...
   * @throws {HTTPValidationError} Validation Error
   */
  return async (id: string, requestOptions?: RequestOptions): Promise<CheckoutLink> => {
    const pathParams = {
      id: id,
    };
    const queryParams = {};
