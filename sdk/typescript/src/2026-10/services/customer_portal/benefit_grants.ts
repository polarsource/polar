...
export const iterListBenefitGrants = (client: ClientBase) => {
  return async function* (
    ...
    while (true) {
      const response = await listBenefitGrants(client)({ ...query, page, limit }, requestOptions);
...
export const getBenefitGrants = (client: ClientBase) => {
   ...
   * @throws {HTTPValidationError} Validation Error
   */
  return async (id: string, requestOptions?: RequestOptions): Promise<CustomerBenefitGrant> => {
    const pathParams = {
      id: id,
    };
    const queryParams = {};
