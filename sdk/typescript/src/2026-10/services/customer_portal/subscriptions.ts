...
export const iterListSubscriptions = (client: ClientBase) => {
  return async function* (
    ...
    while (true) {
      const response = await listSubscriptions(client)({ ...query, page, limit }, requestOptions);
...
export const getSubscriptions = (client: ClientBase) => {
   ...
   * @throws {HTTPValidationError} Validation Error
   */
  return async (id: string, requestOptions?: RequestOptions): Promise<CustomerSubscription> => {
    const pathParams = {
      id: id,
    };
    const queryParams = {};
