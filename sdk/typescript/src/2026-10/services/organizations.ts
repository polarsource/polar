...
export const iterListOrganizations = (client: ClientBase) => {
  return async function* (
    ...
    while (true) {
      const response = await listOrganizations(client)({ ...query, page, limit }, requestOptions);
...
export const getOrganizations = (client: ClientBase) => {
   ...
   * @throws {HTTPValidationError} Validation Error
   */
  return async (id: string, requestOptions?: RequestOptions): Promise<Organization> => {
    const pathParams = {
      id: id,
    };
    const queryParams = {};
