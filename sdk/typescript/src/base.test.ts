...
describe("sendRequest", () => {
  ...
  test("uses a per-request timeout override", async () => {
    ...
    await client.sendRequest(["https://api.polar.sh/v1/items/", {}], { timeout: 30 });
  ...
  test("combines a request signal with the timeout signal", async () => {
    ...
    await client.sendRequest(["https://api.polar.sh/v1/items/", { signal: requestSignal }]);
  ...
  test.each([-1, Number.NaN, Number.POSITIVE_INFINITY, 2_147_483.648])(
    ...
    async (timeout) => {
      ...
      await expect(
        client.sendRequest(["https://api.polar.sh/v1/items/", {}], { timeout }),
