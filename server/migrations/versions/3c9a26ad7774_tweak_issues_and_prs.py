...
def upgrade() -> None:
    ...
    op.drop_constraint(
        'issues_organization_name_repository_name_number_key', 'issues', type_='unique',
    )
    op.drop_constraint('issues_token_key', 'issues', type_='unique')
    op.create_unique_constraint(
        None, 'issues', ['organization_id', 'repository_id', 'number'],
    )
    ...
    op.create_unique_constraint(
        None, 'pull_requests', ['organization_id', 'repository_id', 'number'],
    )
...
def downgrade() -> None:
    ...
    op.add_column('pull_requests', sa.Column(
        'repository_name', sa.VARCHAR(), autoincrement=False, nullable=False,
    ))
    op.add_column('pull_requests', sa.Column(
        'is_locked', sa.BOOLEAN(), autoincrement=False, nullable=False,
    ))
    op.add_column('pull_requests', sa.Column(
        'lock_reason', sa.VARCHAR(), autoincrement=False, nullable=True,
    ))
    op.add_column('pull_requests', sa.Column(
    ...
    op.alter_column(
        'pull_requests', 'is_draft', existing_type=sa.BOOLEAN(), nullable=False,
    )
    op.add_column('issues', sa.Column(
        'token', sa.VARCHAR(), autoincrement=False, nullable=False,
    ))
    op.add_column('issues', sa.Column(
        'is_locked', sa.BOOLEAN(), autoincrement=False, nullable=False,
    ))
