                            ...
                            with tag.div(
                                ...
                                with action_bar(position="right"):
                                    with button(
                                        ...
                                        size="sm",
                                        ghost=True,
                                        hx_get=str(request.url_for("organizations-v2:impersonate", organization_id=self.org.id, user_id=member.user_id)),
                                    ):
                                        text("Impersonate")
                                        ...
                                        with tag.ul(
                                            ...
                                            **{"tabindex": "0"},
                                            ...
                                            if not member_is_admin:
                                                with tag.li():
                                                    with tag.a(
                                                        hx_post=str(request.url_for("organizations-v2:make_admin", organization_id=self.org.id, user_id=member.user_id)),
                                                        hx_confirm="Make this user an admin?",
                                                    ):
                                                        text("Make Admin")
                                            with tag.li():
                                                with tag.a(
                                                    hx_delete=str(request.url_for("organizations-v2:remove_member", organization_id=self.org.id, user_id=member.user_id)),
