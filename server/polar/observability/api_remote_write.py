"""
Prometheus Remote Write client for pushing API server metrics.

This is a simplified version of the worker remote_write.py that doesn't
include queue metrics collection (which is worker-specific).
    ...
    for attempt in range(max_retries):
        try:
            ...
            if response.status_code >= 500 and attempt < max_retries - 1:
                log.warning(
                    "api_prometheus_remote_write_retry",
            ...
            log.warning(
                "api_prometheus_remote_write_failed",
                status_code=response.status_code,
            ...
            if attempt < max_retries - 1:
                ...
                time.sleep(min(2**attempt, 10))
                continue
            log.error("api_prometheus_remote_write_network_error", error=str(e))
    ...
    with httpx.Client(timeout=httpx.Timeout(30.0, connect=5.0)) as client:
        while not shutdown_event.is_set():
            ...
            try:
                ...
                log.error(
                    "api_prometheus_remote_write_error",
                    error=str(e),
...
def start_api_remote_write_pusher() -> bool:
    """
    Start the background thread that pushes API metrics to Prometheus.

    Returns True if the pusher was started, False if disabled or already running.
    """
    ...
    with _start_lock:
        ...
        if not url:
            log.info("api_prometheus_remote_write_disabled", reason="no URL configured")
            return False

        if settings.PROMETHEUS_REMOTE_WRITE_INTERVAL <= 0:
            log.error(
                "api_prometheus_remote_write_invalid_config",
        ...
        _pusher_thread = threading.Thread(
            ...
            daemon=True,
            name="api-prometheus-remote-write",
        )
        _pusher_thread.start()

        log.info(
            "api_prometheus_remote_write_started",
            url=url,
...
def stop_api_remote_write_pusher(timeout: float = 5.0) -> None:
    """
    Stop the background thread that pushes API metrics.
    ...
    with _start_lock:
        ...
        log.info("api_prometheus_remote_write_stopping")
        _shutdown_event.set()
    ...
    with _start_lock:
        if thread.is_alive():
            log.warning("api_prometheus_remote_write_stop_timeout")
        else:
            log.info("api_prometheus_remote_write_stopped")
