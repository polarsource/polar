...
# A payment lock held longer than this is treated as wedged. It's set on an
# off-session charge attempt and normally cleared within seconds by the
# resolving Stripe webhook (success/failure). An hour comfortably exceeds a
# healthy charge + webhook round-trip while still recovering wedged orders the
# same day, well before the next hourly dunning run would repeatedly dead-letter.
STALE_PAYMENT_LOCK_THRESHOLD = timedelta(hours=1)
...
async def process_stale_payment_locks() -> None:
    ...
    async with AsyncSessionMaker() as session:
        ...
        stale_orders = await order_repository.get_stale_payment_lock_orders(
            utc_now() - STALE_PAYMENT_LOCK_THRESHOLD
        )
