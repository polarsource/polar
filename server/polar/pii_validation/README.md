...
2. `scripts.emit_pii_validation` checks its release, then calls the same
   `configure_logfire("server")` and `configure_logging(logfire=True)` functions
   as the API.
3. The script emits synthetic structured fields, a stdlib message, an exception,
   and a direct Logfire record. It flushes the exporters before printing a safe
   manifest and exiting.
4. GitHub retrieves that manifest from the job's stored Render logs. It checks the
   release/environment and requires Logfire and S3 to be enabled.
5. The verifier reads the **one-off job's** Render logs, Logfire rows, and S3 gzip
   batches. It scans full returned payloads, including nested
...
frontend telemetry, Sentry, or PostHog. It does not change billing data or send email.
...
Render job inherits the API's normal exporter credentials and needs none of these
read credentials.
...
All three destinations are required. Disabled exporters must fail, not silently
