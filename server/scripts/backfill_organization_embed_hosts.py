"""
Backfill organizations.embed_hosts to an empty list.

The column was added as nullable to avoid a locking migration on the hot
`organizations` table. New rows get `[]` from the model default, but rows that
...
Usage:
    ...
    uv run python -m scripts.backfill_organization_embed_hosts

    # Execute the backfill (batched):
    uv run python -m scripts.backfill_organization_embed_hosts --execute
"""
    ...
    try:
        async with sessionmaker() as session:
            result = await session.execute(
                select(func.count()).where(Organization.embed_hosts.is_(None))
            )
            total = result.scalar_one()

        if total == 0:
            console.print("[green]No organizations with a NULL embed_hosts.")
            return

        if not execute:
            console.print(
                f"[yellow]Dry-run — use --execute to backfill {total} "
                "organization(s) to an empty embed_hosts."
            )
            return
        ...
        subquery = (
            ...
            .where(Organization.embed_hosts.is_(None))
        ...
        console.print(
            f"\n[green]Backfilled {rows_updated} organization(s) to an empty "
            "embed_hosts."
        )
