"""Backfill the shared `tiers` column on seat-based product prices.

Translates each row's legacy `seat_tiers` (inclusive seat ranges) to the
shared tiers format (per-tier `up_to` bounds plus a `minimum_units` floor)
with `seat_tiers_to_tiers_data` — the same function the dual-write hook
uses for new rows, so backfilled and freshly written rows are
byte-identical.
...
async def _run(session: AsyncSession, *, batch_size: int, dry_run: bool) -> int:
    ...
    while True:
        ...
        for price in prices:
            ...
            # canonical column.
            validate_tiers_data(tiers_data)
