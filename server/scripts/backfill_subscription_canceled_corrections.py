...
def _needs_corrective_event(
    ...
    subscription_canceled_day = subscription.canceled_at.astimezone(UTC).date()
    latest_event_canceled_day = (
        latest_event.canceled_at.astimezone(UTC).date()
        if latest_event.canceled_at is not None
        else None
    )

    if latest_event_canceled_day != subscription_canceled_day:
        return True
