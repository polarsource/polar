...
async def backfill_order_paid_metadata(
    ...
    discount_amount, discount_id, platform_fee, subscription_id, subscription_type
    """
    ...
    with Progress() as progress:
        ...
        while True:
            ...
            if last_id is not None:
                statement = statement.where(Event.id > last_id)
                ...
                if order.subscription_id:
                    ...
                    if order.subscription:
                        ...
                        metadata["subscription_type"] = (
                            f"{sub.recurring_interval.value}"
                            if sub.recurring_interval_count == 1
                            else f"{sub.recurring_interval_count}_{sub.recurring_interval.value}"
                        )
    ...
    with Progress() as progress:
        ...
        while True:
            ...
            for event in events:
                ...
                if not sub_id or sub_id not in subs_by_id:
                    continue
        ...
        while True:
            ...
            if last_created_at is not None:
                statement = statement.where(
                    or_(
                        Subscription.created_at > last_created_at,
                        and_(
                            Subscription.created_at == last_created_at,
            ...
            last_created_at = subscriptions[-1].created_at
...
async def create_missing_subscription_canceled_events(
    ...
    with Progress() as progress:
        ...
        while True:
            ...
            if last_canceled_at is not None:
                statement = statement.where(
                    or_(
                        Subscription.canceled_at > last_canceled_at,
                        and_(
                            Subscription.canceled_at == last_canceled_at,
                            Subscription.id > last_id,
                        ),
    ...
    with Progress() as progress:
        ...
        while True:
            ...
            if last_created_at is not None:
                statement = statement.where(
                    or_(
                        Checkout.created_at > last_created_at,
                        and_(
                            Checkout.created_at == last_created_at,
                            Checkout.id > last_id,
                        ),
            ...
            last_created_at = checkouts[-1].created_at
            last_id = checkouts[-1].id
...
async def backfill(
    ...
    3. Creates subscription.created events for existing subscriptions
    4. Creates subscription.canceled events for canceled subscriptions
    5. Creates checkout.created events for existing checkouts
    """
