...
Note: `free` was removed from `ProductPriceAmountType`, so we filter on the raw
`"free"` literal that legacy rows still carry in the `amount_type` column.
"""
...
subquery = (
    ...
    .where(ProductPrice.amount_type == "free")
