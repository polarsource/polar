...
def is_preview_tinybird_configured() -> bool:
    return bool(
        (os.getenv("PREVIEW_TINYBIRD_API_URL") or "").strip()
        and (os.getenv("PREVIEW_TINYBIRD_ADMIN_TOKEN") or "").strip()
        and (os.getenv("PREVIEW_TINYBIRD_WORKSPACE") or "").strip()
    )
...
def create_or_update(
    ...
    if not dry_run:
        ...
        if is_preview_tinybird_configured():
...
def reset_data(
    ...
    tinybird_result: PreviewTinybirdProvisionResult | None = None
    if not dry_run:
        tinybird_config: PreviewTinybirdAdminConfig | None = None
        if is_preview_tinybird_configured():
        ...
        if tinybird_config is not None:
            tinybird_result = provision_preview_tinybird(
                resolved_preview_id,
                config=tinybird_config,
            )
