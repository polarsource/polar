...
class TestIncrementalAggregation:
    async def test_last_balanced_event_fresh_session(
        self,
        save_fixture: SaveFixture,
        session: AsyncSession,
        locker: Locker,
        ...
        customer_meter = CustomerMeter(
            customer=customer,
            meter=meter,
            last_balanced_event=first_event,
            consumed_units=Decimal(10),
            credited_units=0,
            balance=Decimal(-10),
            activated_at=utc_now(),
        ...
        async with SQLAAsyncSession(
            bind=connection, expire_on_commit=False
        ) as fresh_session:
            updated_meter, updated = await customer_meter_service.update_customer_meter(
                cast(AsyncSession, fresh_session), locker, customer, meter
            )

            assert updated_meter is not None
            assert updated is True
            assert updated_meter.consumed_units == Decimal(15)

    async def test_incremental_sum_aggregation(
        self,
        save_fixture: SaveFixture,
        session: AsyncSession,
        locker: Locker,
        ...
        assert updated_meter is not None
        assert updated is True
        assert updated_meter.consumed_units == Decimal(30)

    async def test_incremental_count_aggregation(
        self,
        save_fixture: SaveFixture,
        session: AsyncSession,
        locker: Locker,
        ...
        """Test that incremental COUNT works correctly without type errors."""
        ...
        count_meter = await create_meter(
            ...
            filter=Filter(
                ...
                clauses=[
                    FilterClause(
                        property="type", operator=FilterOperator.eq, value="request"
                    )
                ],
            ),
        ...
        updated_meter, updated = await customer_meter_service.update_customer_meter(
            session, locker, customer, count_meter
        )

        assert updated_meter is not None
        assert updated is True
        assert updated_meter.consumed_units == Decimal(3)

    async def test_incremental_count_with_external_id(
        self,
        save_fixture: SaveFixture,
        session: AsyncSession,
        locker: Locker,
        ...
        """
        Test incremental COUNT with external_id customer.

        This exercises the UNION code path where SUM(bigint) returns numeric/Decimal,
        which previously caused a type mismatch error with float.
        """
        from polar.meter.aggregation import CountAggregation

        count_meter = await create_meter(
            save_fixture,
            name="Request Count External",
            filter=Filter(
                conjunction=FilterConjunction.and_,
                clauses=[
                    FilterClause(
                        property="type", operator=FilterOperator.eq, value="api_call"
                    )
                ],
            ),
            aggregation=CountAggregation(),
        ...
        updated_meter, updated = await customer_meter_service.update_customer_meter(
            session, locker, customer_with_external_id, count_meter
        )
    ...
    async def test_incremental_sum_with_external_id(
        self,
        save_fixture: SaveFixture,
        session: AsyncSession,
        locker: Locker,
        ...
        sum_meter = await create_meter(
            save_fixture,
            name="Token Sum External",
            filter=Filter(
                conjunction=FilterConjunction.and_,
                clauses=[
                    FilterClause(
                        property="type", operator=FilterOperator.eq, value="usage"
                    )
                ],
            ),
            aggregation=PropertyAggregation(
        ...
        first_event = await create_event(
            ...
            organization=organization,
            customer=customer_with_external_id,
            metadata={"type": "usage", "tokens": 100},
        )
        customer_meter = CustomerMeter(
            customer=customer_with_external_id,
            meter=sum_meter,
        ...
        await create_event(
            ...
            organization=organization,
            customer=customer_with_external_id,
            metadata={"type": "usage", "tokens": 50},
        )

        updated_meter, updated = await customer_meter_service.update_customer_meter(
            session, locker, customer_with_external_id, sum_meter
        )

        assert updated_meter is not None
        assert updated is True
        assert updated_meter.consumed_units == Decimal(150)

    async def test_incremental_max_aggregation(
        self,
        save_fixture: SaveFixture,
        session: AsyncSession,
        locker: Locker,
        ...
        updated_meter, updated = await customer_meter_service.update_customer_meter(
            session, locker, customer, max_meter
        )

        assert updated_meter is not None
        assert updated is True
        assert updated_meter.consumed_units == Decimal(80)

    async def test_incremental_min_aggregation(
        self,
        save_fixture: SaveFixture,
        session: AsyncSession,
        locker: Locker,
        ...
        min_meter = await create_meter(
            ...
            filter=Filter(
                ...
                clauses=[
                    FilterClause(
                        property="type", operator=FilterOperator.eq, value="latency"
                    )
                ],
            ),
            aggregation=PropertyAggregation(
                func=AggregationFunction.min, property="value"
            ),
            organization=organization,
        )
        ...
        first_event = await create_event(
            ...
            organization=organization,
            customer=customer,
            metadata={"type": "latency", "value": 100},
        )
        customer_meter = CustomerMeter(
            customer=customer,
            meter=min_meter,
        ...
        await create_event(
            ...
            organization=organization,
            customer=customer,
            metadata={"type": "latency", "value": 30},
        )

        updated_meter, updated = await customer_meter_service.update_customer_meter(
            session, locker, customer, min_meter
        )
