import pytest
from httpx import AsyncClient

from polar.config import settings
from polar.kit import jwt
from polar.models import Customer, Organization
from tests.fixtures.auth import CUSTOMER_AUTH_SUBJECT, MEMBER_AUTH_SUBJECT
from tests.fixtures.database import SaveFixture
from tests.fixtures.random_objects import create_customer


@pytest.mark.asyncio
class TestAuthorize:
    async def test_anonymous_with_customer_id_is_rejected(
        self,
        client: AsyncClient,
        save_fixture: SaveFixture,
        organization: Organization,
    ) -> None:
        """
        An anonymous caller must NOT be able to initiate an OAuth flow for an
        arbitrary customer. If allowed, the callback creates a customer session
        for whatever `customer_id` ends up in the signed state — a full account
        takeover given any customer UUID.
        """
        victim = await create_customer(
            save_fixture, organization=organization, email="victim@example.com"
        )
        response = await client.get(
            "/v1/customer-portal/oauth-accounts/authorize",
            params={
                "platform": "github",
                "return_to": "/",
                "customer_id": str(victim.id),
            },
        )
        assert response.status_code == 401

    async def test_anonymous_without_customer_id_is_rejected(
        self, client: AsyncClient
    ) -> None:
        """Anonymous callers are rejected regardless of whether they pass a customer_id."""
        response = await client.get(
            "/v1/customer-portal/oauth-accounts/authorize",
            params={"platform": "github", "return_to": "/"},
        )
        assert response.status_code == 401

    @pytest.mark.auth(CUSTOMER_AUTH_SUBJECT)
    async def test_customer_cannot_override_customer_id(
        self,
        client: AsyncClient,
        save_fixture: SaveFixture,
        customer: Customer,
        organization: Organization,
    ) -> None:
        """
        Even an authenticated customer must not be able to initiate a flow for
        another customer by passing a different `customer_id` query param. The
        state JWT must encode the authenticated subject's customer_id.
        """
        other_customer = await create_customer(
            save_fixture, organization=organization, email="other@example.com"
        )
        response = await client.get(
            "/v1/customer-portal/oauth-accounts/authorize",
            params={
                "platform": "github",
                "return_to": "/",
                "customer_id": str(other_customer.id),
            },
        )
        assert response.status_code == 200

        data = response.json()
        assert "url" in data

        state = _extract_state(data["url"])
        decoded = jwt.decode(token=state, secret=settings.SECRET, type="customer_oauth")
        assert decoded["customer_id"] == str(customer.id)
        assert decoded["customer_id"] != str(other_customer.id)

    @pytest.mark.auth(CUSTOMER_AUTH_SUBJECT)
    async def test_customer_without_customer_id_succeeds(
        self, client: AsyncClient, customer: Customer
    ) -> None:
        """customer_id query param is optional for authenticated callers."""
        response = await client.get(
            "/v1/customer-portal/oauth-accounts/authorize",
            params={"platform": "github", "return_to": "/"},
        )
        assert response.status_code == 200
        data = response.json()
        state = _extract_state(data["url"])
        decoded = jwt.decode(token=state, secret=settings.SECRET, type="customer_oauth")
        assert decoded["customer_id"] == str(customer.id)

    @pytest.mark.auth(MEMBER_AUTH_SUBJECT)
    @pytest.mark.keep_session_state
    async def test_member_cannot_override_customer_id(
        self,
        client: AsyncClient,
        save_fixture: SaveFixture,
        organization: Organization,
    ) -> None:
        """
        An authenticated member must not be able to initiate a flow for another
        customer. The state JWT must encode the member's associated customer_id,
        never the caller-supplied value.
        """
        attacker_customer = await create_customer(
            save_fixture,
            organization=organization,
            email="attacker-target@example.com",
        )
        response = await client.get(
            "/v1/customer-portal/oauth-accounts/authorize",
            params={
                "platform": "github",
                "return_to": "/",
                "customer_id": str(attacker_customer.id),
            },
        )
        assert response.status_code == 200

        data = response.json()
        state = _extract_state(data["url"])
        decoded = jwt.decode(token=state, secret=settings.SECRET, type="customer_oauth")
        # Must be the member's own customer_id — not the caller-supplied one.
        assert decoded["customer_id"] != str(attacker_customer.id)
        assert "member_id" in decoded


def _extract_state(authorization_url: str) -> str:
    from urllib.parse import parse_qs, urlparse

    parsed = urlparse(authorization_url)
    qs = parse_qs(parsed.query)
    assert "state" in qs, f"state not found in URL: {authorization_url}"
    return qs["state"][0]
