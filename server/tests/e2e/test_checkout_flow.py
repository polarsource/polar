...
Tests the high-level flow:
1. Create checkout for a product via HTTP API
2. Customer confirms checkout via HTTP API
3. Tasks execute (order created)
4. Verify order and customer are created

These tests execute actual Dramatiq tasks via StubBroker.
"""
...
from polar.models import Account, Order, Organization, Product, User
...
class TestCheckoutFlow:
    ...
    async def test_checkout_creates_order(
        ...
        # 6. Email verification (optional - depends on email service configuration)
        # The important assertions (order created, customer correct) have passed
