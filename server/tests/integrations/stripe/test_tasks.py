...
class TestPaymentIntentSucceeded:
    async def test_retry_payment_saves_payment_method_and_updates_subscription(
        ...
        mocker.patch(
            "polar.integrations.stripe.service.stripe.set_automatically_charged_subscription"
        )
    ...
    async def test_polar_managed_subscription_only_updates_local_record(
        self,
        mocker: MockerFixture,
        session: AsyncSession,
        save_fixture: SaveFixture,
        ...
        subscription.stripe_subscription_id = None  # Polar-managed
        subscription.payment_method = old_payment_method
        await save_fixture(subscription)
        ...
        # Should NOT call Stripe subscription update for Polar-managed
        ...
        # Then: Local subscription is updated but Stripe subscription is not called
        subscription_repo = SubscriptionRepository.from_session(session)
        updated_subscription = await subscription_repo.get_by_id(
            subscription.id,
            options=(selectinload(subscription_repo.model.payment_method),),
        ...
        stripe_subscription_mock.assert_not_called()
