...
class TestTriggerPayment:
    ...
    async def test_create_manual_retry_payment_intent_success(
        self,
        stripe_service_mock: MagicMock,
        session: AsyncSession,
        save_fixture: SaveFixture,
        ...
        """Test successful creation of manual retry payment intent."""
        # Given
        subscription = await create_active_subscription(
            save_fixture, product=product, customer=customer
        )
        ...
        mock_payment_intent = SimpleNamespace(
            id="pi_test_123",
            client_secret="pi_test_123_secret_abc",
            status="requires_payment_method",
        )
        stripe_service_mock.create_payment_intent.return_value = mock_payment_intent

        # When
        payment = await order_service.create_manual_retry_payment_intent(session, order)

        # Then
        stripe_service_mock.create_payment_intent.assert_called_once_with(
    ...
    async def test_create_manual_retry_payment_intent_not_pending(
        self,
        stripe_service_mock: MagicMock,
        session: AsyncSession,
        save_fixture: SaveFixture,
        ...
        """Test that manual retry fails for non-pending orders."""
        # Given
        subscription = await create_active_subscription(
            save_fixture, product=product, customer=customer
        )
        ...
        # When/Then
        with pytest.raises(OrderNotPending):
            await order_service.create_manual_retry_payment_intent(session, order)

        stripe_service_mock.create_payment_intent.assert_not_called()

    async def test_create_manual_retry_payment_intent_not_eligible(
        self,
        stripe_service_mock: MagicMock,
        session: AsyncSession,
        save_fixture: SaveFixture,
        ...
        """Test that manual retry fails for orders without retry scheduled."""
        # Given
        subscription = await create_active_subscription(
            save_fixture, product=product, customer=customer
        )
    ...
    async def test_create_manual_retry_payment_intent_payment_already_in_progress(
        self,
        stripe_service_mock: MagicMock,
        session: AsyncSession,
        save_fixture: SaveFixture,
        ...
        """Test that manual retry fails when payment is already in progress."""
        # Given
        subscription = await create_active_subscription(
            save_fixture, product=product, customer=customer
        )
