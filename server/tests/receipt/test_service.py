...
@pytest.mark.asyncio
class TestBulkAllocateForCustomer:
    async def test_allocates_sequential_numbers(
        self,
        session: AsyncSession,
        save_fixture: SaveFixture,
        organization: Organization,
        ...
        allocated = await receipt_service.bulk_allocate_for_customer(
        ...
        sequence = sorted(
            int(o.receipt_number.split("-")[-1])
            for o in orders
            if o.receipt_number is not None
        )
        assert sequence == [1, 2, 3]

        await session.refresh(customer)
        assert customer.receipt_next_number == 4

    async def test_skips_already_allocated_orders(
        self,
        session: AsyncSession,
        save_fixture: SaveFixture,
        organization: Organization,
        ...
        already = await create_order(save_fixture, customer=customer)
        await create_payment(save_fixture, organization, order=already)
        already.receipt_number = "RCPT-PRESET-0042"
        await save_fixture(already)

        new_order = await create_order(save_fixture, customer=customer)
        await create_payment(save_fixture, organization, order=new_order)

        allocated = await receipt_service.bulk_allocate_for_customer(
            session, customer, [already, new_order]
        )
        ...
        assert already.receipt_number == "RCPT-PRESET-0042"
        assert new_order.receipt_number is not None

    async def test_returns_zero_for_empty_input(
        self,
        session: AsyncSession,
        customer: Customer,
    ) -> None:
