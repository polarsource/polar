...
class TestBranchEndpoints:
    async def test_lifecycle(
        ...
        assert branch["base_deployment_id"] == deployed["id"]
        assert branch["version_id"] == deployed["version_id"]
        assert branch["deployment_id"] == deployed["id"]

        response = await void_client.patch(
            f"{PATH}/{branch['id']}", headers=HEADERS, json={"patch": PATCH}
        )
        ...
        response = await void_client.post(
            f"{PATH}/{branch['id']}/promote", headers=HEADERS
        )
        assert response.status_code == 201
        promoted = response.json()
        assert promoted["status"] == "draft"
        assert promoted["checksum"] == f"branch:{branch['id']}"
        ...
        assert (
            await void_client.delete(f"{PATH}/{branch['id']}", headers=HEADERS)
        ).status_code == 204
        assert (
            await void_client.get(f"{PATH}/{branch['id']}", headers=HEADERS)
        ).status_code == 404
