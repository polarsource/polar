...
@pytest.mark.asyncio
class TestCountPendingEventsBefore:
    async def test_single_event(
        ...
        assert await webhook_service.count_pending_events_before(session, event) == 0
    ...
    async def test_undelivered_previous_event(
        ...
        assert await webhook_service.count_pending_events_before(session, event) == 1
